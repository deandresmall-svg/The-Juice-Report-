...
def build_binary_projection_snapshot(
    ...
    snapshot["SportsbookOdds"] = np.nan
    snapshot["MarketProbability"] = np.nan
    snapshot["MarketImpliedProbability"] = np.nan
    snapshot["ModelMarketEdge"] = np.nan
    ...
    preferred = [
        ...
        "Calibration_Version", "SportsbookOdds", "MarketProbability",
...
quick_tab, contact_tab, matchup_tab, bvp_tab, pitcher_tab, backtest_tab, notes_tab = st.tabs(
    [
        "Quick board", "Contact profile", "Matchup detail", "Batter vs pitcher",
        "Pitcher profile", "Backtest & calibration", "Model notes"
    ]
)
...
with quick_tab:
    ...
    styler = quick.style.background_gradient(
        ...
        "Order": "{:.0f}", "Proj PA": "{:.2f}", "1+ Hit": "{:.1%}", "Raw 1+ Hit": "{:.1%}",
