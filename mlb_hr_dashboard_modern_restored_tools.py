...
def inject_clean_css() -> None:
    st.markdown(
        ...
        :root {
            ...
            --line-bright:rgba(96, 165, 250, .38);
            --text:#f8fafc;
            --muted:#9fb3c8;
            --blue:#2f7df6;
            --cyan:#20d3ee;
        ...
        html, body, [data-testid="stAppViewContainer"], [data-testid="stMain"], .stApp {
            background:
                radial-gradient(circle at 6% 0%, rgba(47,125,246,.24), transparent 26rem),
                radial-gradient(circle at 94% 2%, rgba(139,92,246,.20), transparent 25rem),
                linear-gradient(180deg, var(--bg-0) 0%, var(--bg-1) 45%, #061426 100%) !important;
            color:var(--text) !important;
        }
        [data-testid="stHeader"] {
            background:rgba(3, 17, 31, .78) !important;
        ...
        [data-testid="stSidebar"] {
            background:linear-gradient(180deg, #061426 0%, #09213d 48%, #07182d 100%) !important;
        ...
        [data-testid="stSidebar"] [data-baseweb="select"] * { color:#e5f1ff !important; }
        [data-testid="stSidebar"] .stButton button {
            background:linear-gradient(135deg, rgba(37,99,235,.26), rgba(14,165,233,.18)) !important;
            border-color:rgba(96,165,250,.35) !important;
        }
        h1, h2, h3, h4, h5, h6, p, label, span, div { color:inherit; }
        h1 { letter-spacing:-.045em; font-weight:950; }
        h2, h3 { letter-spacing:-.028em; font-weight:900; }
        ...
        .sidebar-brand {
            ...
            margin:.2rem 0 .85rem;
            border-radius:18px;
            background:linear-gradient(145deg, rgba(37,99,235,.22), rgba(15,42,72,.45));
        ...
        .brand-logo {
            ...
            color:#0b2442 !important; font-size:1.45rem; font-weight:950;
            box-shadow:0 10px 26px rgba(59,130,246,.28);
        }
        .brand-copy b { display:block; color:#fff !important; font-size:1.02rem; letter-spacing:.02em; }
        .brand-copy small { color:#93c5fd !important; font-weight:750; letter-spacing:.18em; font-size:.62rem; }
        .modern-hero {
            position:relative;
            overflow:hidden;
            border-radius:28px;
            ...
            background:
                linear-gradient(135deg, rgba(15,42,72,.96) 0%, rgba(7,27,51,.96) 58%, rgba(12,33,60,.96) 100%);
            border:1px solid rgba(96,165,250,.25);
            box-shadow:0 24px 65px rgba(0,0,0,.30);
        }
        .modern-hero:before {
            content:""; position:absolute; inset:-40% -20% auto auto; width:520px; height:520px;
            background:radial-gradient(circle, rgba(47,125,246,.28), transparent 60%);
            pointer-events:none;
        }
        .modern-hero-row { position:relative; display:flex; align-items:center; justify-content:space-between; gap:1rem; }
        .modern-title-wrap { display:flex; align-items:center; gap:.9rem; }
        .modern-badge {
            display:grid; place-items:center;
            width:54px; height:54px; border-radius:16px;
            background:linear-gradient(135deg,#ffffff,#dbeafe);
            color:#0b2442 !important; font-size:1.75rem;
            box-shadow:0 16px 36px rgba(47,125,246,.25);
        }
        .modern-title { color:#fff !important; margin:0; font-size:1.75rem; font-weight:950; letter-spacing:-.045em; }
        .modern-subtitle { color:#a9bed6 !important; margin:.18rem 0 0; font-size:.94rem; }
        .hero-pills { display:flex; flex-wrap:wrap; justify-content:flex-end; gap:.55rem; }
        .hero-pill {
        ...
        .app-kicker {
            ...
            color:white !important; font-size:.72rem; font-weight:900;
            letter-spacing:.11em; text-transform:uppercase;
            box-shadow:0 8px 20px rgba(37,99,235,.24); margin-bottom:.35rem;
        }
        div[data-testid="stMetric"] {
            position:relative;
            background:linear-gradient(145deg, rgba(15,42,72,.98), rgba(10,31,56,.94)) !important;
        ...
        div[data-testid="stMetric"]:before {
            content:""; position:absolute; right:-34px; top:-34px; width:92px; height:92px; border-radius:50%;
            background:radial-gradient(circle, rgba(47,125,246,.28), transparent 62%);
        }
        div[data-testid="stMetricLabel"] p { color:#bed4ec !important; font-weight:800 !important; }
        ...
        [data-testid="stDataFrame"], div[data-testid="stTable"] {
            ...
            background:rgba(9,31,56,.70);
        }
        div[data-testid="stDataFrame"] div[role="grid"] { border-radius:18px; }
        .stDataFrame, .stTable { color:#0f172a; }
        ...
        div[data-testid="stExpander"] {
            background:rgba(9,31,56,.72) !important;
            border:1px solid rgba(96,165,250,.22) !important;
            border-radius:18px !important;
            box-shadow:0 12px 34px rgba(0,0,0,.16);
        }
        div[data-testid="stExpander"] summary { color:#f8fafc !important; font-weight:850; }
        ...
        .stButton button[kind="secondary"], [data-testid="stDownloadButton"] button[kind="secondary"] {
            background:rgba(15,42,72,.74) !important;
        }
        input, textarea, [data-baseweb="select"] > div, [data-baseweb="input"] > div,
        [data-baseweb="base-input"], [data-testid="stNumberInput"] input {
            background:rgba(15,42,72,.84) !important;
            color:#e8f2ff !important;
            border-color:rgba(148,163,184,.26) !important;
            border-radius:12px !important;
        }
        [data-baseweb="select"] span, [data-baseweb="select"] svg { color:#e8f2ff !important; fill:#e8f2ff !important; }
        .stSlider [data-baseweb="slider"] { color:#60a5fa !important; }
        div[role="radiogroup"] { gap:.58rem; flex-wrap:wrap; }
        div[role="radiogroup"] label {
            background:rgba(15,42,72,.78);
        ...
        div[role="radiogroup"] label:hover { border-color:#60a5fa; transform:translateY(-1px); }
        .slate-card {
            min-height:118px;
            background:linear-gradient(145deg, rgba(15,42,72,.98), rgba(8,29,54,.94));
            border:1px solid rgba(148,163,184,.24);
            border-radius:18px;
            padding:.78rem .72rem .66rem;
            text-align:center;
            box-shadow:0 14px 34px rgba(0,0,0,.20);
            transition:.16s ease;
        }
        .slate-card:hover { transform:translateY(-2px); border-color:rgba(96,165,250,.46); }
        .slate-card.selected, .slate-card selected {
            border:2px solid #38bdf8;
            background:linear-gradient(145deg, rgba(14,165,233,.18), rgba(15,42,72,.98));
            box-shadow:0 18px 40px rgba(14,165,233,.22);
        }
        .slate-time, .slate-venue, .leader-sub, .hero-meta, .park-source, .context-note { color:#a9bed6 !important; }
        .slate-game, .leader-name, .hero-title { color:#fff !important; font-weight:900; }
        .leader-card, .matchup-hero, .context-note, .park-grid, .offense-choice {
            background:linear-gradient(145deg, rgba(15,42,72,.94), rgba(8,29,54,.86));
            border:1px solid rgba(96,165,250,.22);
            border-radius:18px;
            box-shadow:0 16px 38px rgba(0,0,0,.20);
        }
        .leader-prob { color:#60a5fa !important; font-weight:950; }
        .leader-rank { background:#2563eb; color:#fff !important; }
        .board-heading { color:#f8fafc !important; font-weight:950; }
        .factor, .park-chip, .hero-badge {
            background:rgba(37,99,235,.16);
            border:1px solid rgba(96,165,250,.26);
            color:#dbeafe !important;
            border-radius:999px;
        }
        hr, [data-testid="stDivider"] { border-color:rgba(148,163,184,.18) !important; }
        ::-webkit-scrollbar { width:11px; height:11px; }
        ::-webkit-scrollbar-track { background:#061426; }
...
# -----------------------------------------------------------------------------
# HR binary target compatibility used by the whole-slate/parlay tools
# -----------------------------------------------------------------------------
...
BINARY_EXPECTED_COUNT_COLUMN = "Projected_HR"
BINARY_EXPECTED_COUNT_LABEL = "Projected HR"
BINARY_PER_PA_COLUMN = "Model_HR_Per_PA"
...
def apply_binary_probability_calibration(board: pd.DataFrame, calibration: dict | None) -> pd.DataFrame:
    """Alias the older whole-slate tools to the modern HR calibration function."""
    return apply_hr_probability_calibration(board, calibration)
    ...
    <div class="modern-hero">
        <div class="modern-hero-row">
            <div class="modern-title-wrap">
                <div class="modern-badge">💥</div>
                <div>
                    <div class="modern-title">Home Run Targeting</div>
                    <div class="modern-subtitle">Actionable power board with HR probability, HRScore, launch-angle fit and backtested buckets.</div>
                </div>
            </div>
            <div class="hero-pills">
                <span class="hero-pill">🚀 HR Probability</span>
                <span class="hero-pill">🏆 HRScore</span>
                <span class="hero-pill">📊 Snapshot Lab</span>
            </div>
        </div>
    </div>
    """,
...
with st.sidebar:
    st.markdown(
        ...
        <div class="sidebar-brand">
            ...
            <div class="brand-copy"><b>BATTED BALL LABS</b><small>ANALYTICS</small></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
quick_tab, power_tab, matchup_tab, pitch_type_tab, tracking_tab, pitcher_tab, slate_tab, backtest_tab, notes_tab = st.tabs(
    [
        ...
        "Bat tracking", "Pitcher profile", "Slate Top 10 + 3-man generator", "Backtest & calibration", "Model notes"
    ]
)
...
with slate_tab:
    render_slate_tools(
        ...
        odds_quotes=pd.DataFrame(),
        odds_source_mode="Consensus sportsbooks",
        widget_prefix=f"{MODEL_KEY}_slate",
        bat_tracking_auto=bat_tracking_auto,
    )
