...
def prepare_data(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df["LA_Bucket"] = pd.cut(
        df["launch_angle"],
        bins=[-90, 8, 20, 32, 35, 90],
        labels=["Too Low", "Low", "Sweet Spot", "HR Optimal", "Too High"],
        include_lowest=True
    )
    df["LA_Power_Score"] = 100 - (pd.to_numeric(df["launch_angle"], errors="coerce") - 26).abs() * 5.0
...
def build_hr_board(
    ...
    recent = recent_hr_form(df, [], end_date)
    ...
    base = base.merge(attack_profile, on="Side", how="left")
    ...
    # Park factor integration (weighted by handedness)
    park_factor = (park_hr_factor_lhb * 0.5 + park_hr_factor_rhb * 0.5) / 100.0
    base["Projected_HR"] = base["HR_PA"] * base["Projected_PA"] * park_factor
    ...
    base["HRScore"] = (
        ...
        + percentile(base.get("LA_Power_Score", 50)) * 0.15
...
def render_batter_odds_section(board: pd.DataFrame, matchup: dict, api_key: str, widget_prefix: str) -> tuple[pd.DataFrame, pd.DataFrame, str]:
    ...
    selected_games = st.multiselect(
        "Games to fetch odds for", [f"{matchup['away_abbr']} @ {matchup['home_abbr']}"],
        default=[f"{matchup['away_abbr']} @ {matchup['home_abbr']}"],
...
with notes_tab:
    st.markdown("Model rewritten with improved weighting, shrinkage and matchup factors.")
