...
def render_three_man_hr_parlay_tab(board: pd.DataFrame, widget_prefix: str = "hr") -> None:
    st.subheader("Automatic 3-man HR parlays")
    st.caption(
        "This uses your current HR board only. It ranks combinations by HR probability, HR Score, "
        "pitch match, zone fit, pitcher-side attack, launch-angle power, and barrel rate. Use odds/price "
        "as the final filter before playing anything."
    )
...
with parlay_tab:
    render_three_man_hr_parlay_tab(rankings, widget_prefix=MODEL_KEY)
