...
def infer_recent_lineup(df: pd.DataFrame, team: str) -> pd.DataFrame:
    ...
    # A recent game can contain pinch hitters and substitutions. Keep only the
    # original nine lineup positions so missing values never reach the slate UI.
    order = order[order["LineupSpot"].between(1, 9)].copy()
    order["player_id"] = pd.to_numeric(order["player_id"], errors="coerce")
    order["LineupSpot"] = pd.to_numeric(order["LineupSpot"], errors="coerce")
    order = order.dropna(subset=["player_id", "LineupSpot"])
    order["player_id"] = order["player_id"].astype(int)
    order["LineupSpot"] = order["LineupSpot"].astype(int)
    return order[["player_id", "LineupSpot"]].reset_index(drop=True)
...
def finalize_fantasy_rankings(board: pd.DataFrame) -> pd.DataFrame:
    if board.empty:
        return board
    output = board.copy()
    # Individual game boards may already contain a local Rank column.
    # Remove it before creating one final ranking for the combined slate.
...
def build_selected_slate(
    ...
    for game in games:
        ...
        for side in ("away", "home"):
            ...
            if lineup.empty:
                lineup = _clean_lineup_seed(infer_recent_lineup(df, statcast_team))
            ...
            # substitutions. Invalid rows are skipped instead of crashing the app.
            for row in lineup.itertuples(index=False):
                lineup_spot = pd.to_numeric(getattr(row, "LineupSpot", np.nan), errors="coerce")
                player_id_value = pd.to_numeric(getattr(row, "player_id", np.nan), errors="coerce")
                if pd.isna(lineup_spot) or pd.isna(player_id_value):
                    continue
...
# -----------------------------------------------------------------------------
# Hitter fantasy-score backtesting and official result collection
# -----------------------------------------------------------------------------
FS_BACKTEST_VERSION = "hitter-fs-backtest-v1"
FS_DEFAULT_SCORING = {
    "Single": 3.0,
    "Double": 6.0,
    "Triple": 10.0,
...
FS_SCORING_COLUMNS = {
    ...
    "Double": "Score_Double",
    "Triple": "Score_Triple",
    "HomeRun": "Score_HomeRun",
    "Run": "Score_Run",
    "RBI": "Score_RBI",
    "Walk": "Score_Walk",
    "HBP": "Score_HBP",
    "StolenBase": "Score_StolenBase",
}
FS_ACTUAL_COMPONENTS = {
    "Single": "Actual_Singles",
    "Double": "Actual_Doubles",
    "Triple": "Actual_Triples",
    "HomeRun": "Actual_HR",
    "Run": "Actual_Runs",
    "RBI": "Actual_RBI",
    "Walk": "Actual_Walks",
    "HBP": "Actual_HBP",
    "StolenBase": "Actual_SB",
}


def _fs_numeric(value: object, default: float = np.nan) -> float:
    numeric = pd.to_numeric(pd.Series([value]), errors="coerce").iloc[0]
    return float(default) if pd.isna(numeric) else float(numeric)


def _fantasy_score_from_row(row: pd.Series | dict) -> float:
    total = 0.0
    found_component = False
    getter = row.get if hasattr(row, "get") else lambda key, default=None: default
    for scoring_key, actual_column in FS_ACTUAL_COMPONENTS.items():
        actual = _fs_numeric(getter(actual_column, np.nan))
        if pd.isna(actual):
            continue
        found_component = True
        points_column = FS_SCORING_COLUMNS[scoring_key]
        points = _fs_numeric(getter(points_column, FS_DEFAULT_SCORING[scoring_key]), FS_DEFAULT_SCORING[scoring_key])
        total += actual * points
    return float(total) if found_component else np.nan


def normalize_fantasy_history(history: pd.DataFrame | None) -> pd.DataFrame:
    if history is None or history.empty:
        return pd.DataFrame()
    result = history.copy()
    aliases = {
        "Date": "SlateDate", "Game_ID": "GamePK", "Player_ID": "player_id",
        "Projection": "Projected_FS", "Rating": "FantasyScoreRating",
        "Actual": "Actual_FS", "Line": "PP_Line",
    }
    ...
    text_defaults = {
        "Player": "", "Team": "", "Opponent": "", "Game": "", "Venue": "",
        "Pitcher": "", "PitcherHand": "", "Confidence_Level": "Unknown",
        "LineupStatus": "Unknown", "LineupSource": "", "PP_Line_Type": "",
        "PP_Line_Source": "", "PP_Line_Updated": "", "PreferredSide": "",
        "ResultStatus": "", "Notes": "", "ModelVersion": "unknown",
    }
    ...
    for column in ["SlateDate", "GameDatetimeUTC", "GeneratedAtUTC"]:
        if column not in result.columns:
            result[column] = pd.NaT if column != "SlateDate" else pd.NaT
    ...
    numeric_columns = [
        "GamePK", "player_id", "LineupSpot", "Projected_PA", "Projected_FS",
        "FantasyScoreRating", "FS_Median", "FS_P75", "FS_P90", "Model_1plus_Hit",
        "Model_1plus_HR", "Proj_Total_Bases", "MatchupRating", "Team Implied Runs",
        "Proj_Singles", "Proj_Doubles", "Proj_Triples", "Proj_HR", "Proj_Runs",
        "Proj_RBI", "Proj_Walks", "Proj_HBP", "Proj_SB", "PP_Line",
        "PP_Over_Prob", "PP_Under_Prob", "PP_Push_Prob", "PreferredProbability",
        "ProjectionEdge", "PP_Over_Multiplier", "PP_Under_Multiplier",
        "Actual_PA", "Actual_AB", "Actual_Hits", "Actual_Singles", "Actual_Doubles",
        "Actual_Triples", "Actual_HR", "Actual_Runs", "Actual_RBI", "Actual_Walks",
        "Actual_HBP", "Actual_SB", "Actual_FS",
        *FS_SCORING_COLUMNS.values(),
    ]
    for column in numeric_columns:
        if column not in result.columns:
    ...
    missing_actual = result["Actual_FS"].isna()
    if missing_actual.any():
        result.loc[missing_actual, "Actual_FS"] = result.loc[missing_actual].apply(
            _fantasy_score_from_row, axis=1
        )
    return result


def build_fantasy_snapshot(
    board: pd.DataFrame,
    ...
    snapshot = board.copy()
    generated = pd.to_datetime(generated_at_utc, utc=True, errors="coerce")
    snapshot["GeneratedAtUTC"] = generated
    snapshot["ModelVersion"] = FS_BACKTEST_VERSION

    comparison_lookup = pd.DataFrame()
    if comparison is not None and not comparison.empty and "player_id" in comparison.columns:
        comparison_lookup = comparison.copy()
        comparison_lookup["player_id"] = pd.to_numeric(comparison_lookup["player_id"], errors="coerce")
        comparison_lookup = comparison_lookup.dropna(subset=["player_id"]).drop_duplicates("player_id", keep="last")
        comparison_lookup = comparison_lookup.set_index("player_id")

    for column, default in {
        "PP_Line": np.nan, "PP_Over_Prob": np.nan, "PP_Under_Prob": np.nan,
        "PP_Push_Prob": np.nan, "ProjectionEdge": np.nan, "PreferredProbability": np.nan,
        "PreferredSide": "", "PP_Line_Type": "", "PP_Line_Source": "",
        "PP_Line_Updated": "", "PP_Over_Multiplier": np.nan,
        "PP_Under_Multiplier": np.nan,
    }.items():
        snapshot[column] = default

    for index, row in snapshot.iterrows():
        player_id = _fs_numeric(row.get("player_id"))
        if pd.isna(player_id) or comparison_lookup.empty or player_id not in comparison_lookup.index:
            continue
        line_row = comparison_lookup.loc[player_id]
        if isinstance(line_row, pd.DataFrame):
            line_row = line_row.iloc[-1]
        line = _fs_numeric(line_row.get("PP Line", line_row.get("Auto_PP_Line")))
        over = _fs_numeric(line_row.get("Over Prob"))
        under = _fs_numeric(line_row.get("Under Prob"))
        push = _fs_numeric(line_row.get("Push Prob"))
        snapshot.at[index, "PP_Line"] = line
        snapshot.at[index, "PP_Over_Prob"] = over
        snapshot.at[index, "PP_Under_Prob"] = under
        snapshot.at[index, "PP_Push_Prob"] = push
        snapshot.at[index, "ProjectionEdge"] = _fs_numeric(line_row.get("Projection Edge"))
        no_push = over + under if pd.notna(over) and pd.notna(under) else np.nan
        if pd.notna(no_push) and no_push > 0:
            snapshot.at[index, "PreferredSide"] = "Over" if over >= under else "Under"
            snapshot.at[index, "PreferredProbability"] = max(over, under) / no_push
        for source_column, target_column in [
            ("PP_Line_Type", "PP_Line_Type"), ("PP_Line_Source", "PP_Line_Source"),
            ("PP_Line_Updated", "PP_Line_Updated"),
            ("PP_Over_Multiplier", "PP_Over_Multiplier"),
            ("PP_Under_Multiplier", "PP_Under_Multiplier"),
        ]:
            if source_column in line_row.index:
                snapshot.at[index, target_column] = line_row.get(source_column)

    for scoring_key, scoring_column in FS_SCORING_COLUMNS.items():
        snapshot[scoring_column] = float(scoring.get(scoring_key, FS_DEFAULT_SCORING[scoring_key]))

    for actual_column in [
        "Actual_PA", "Actual_AB", "Actual_Hits", "Actual_Singles", "Actual_Doubles",
        "Actual_Triples", "Actual_HR", "Actual_Runs", "Actual_RBI", "Actual_Walks",
        "Actual_HBP", "Actual_SB", "Actual_FS",
    ]:
        snapshot[actual_column] = np.nan
    snapshot["ResultStatus"] = ""
    snapshot["Notes"] = ""
    game_times = pd.to_datetime(snapshot["GameDatetimeUTC"], utc=True, errors="coerce")
    snapshot["SnapshotAfterStart"] = (
        bool(pd.notna(generated)) & game_times.notna() & game_times.lt(generated)
    )

    preferred = [
        "SlateDate", "GameDatetimeUTC", "GeneratedAtUTC", "ModelVersion", "GamePK",
        "player_id", "Player", "Team", "Opponent", "Game", "Venue", "Pitcher",
        "PitcherHand", "LineupSpot", "Projected_PA", "LineupStatus", "LineupSource",
        "Confidence_Level", "Projected_FS", "FantasyScoreRating", "FS_Median", "FS_P75",
        "FS_P90", "Model_1plus_Hit", "Model_1plus_HR", "Proj_Total_Bases",
        "MatchupRating", "Team Implied Runs", "Proj_Singles", "Proj_Doubles",
        "Proj_Triples", "Proj_HR", "Proj_Runs", "Proj_RBI", "Proj_Walks", "Proj_HBP",
        "Proj_SB", *FS_SCORING_COLUMNS.values(), "PP_Line", "PP_Over_Prob",
        "PP_Under_Prob", "PP_Push_Prob", "PreferredSide", "PreferredProbability",
        "ProjectionEdge", "PP_Line_Type", "PP_Line_Source", "PP_Line_Updated",
        "PP_Over_Multiplier", "PP_Under_Multiplier", "SnapshotAfterStart",
        "Actual_PA", "Actual_AB", "Actual_Hits", "Actual_Singles", "Actual_Doubles",
        "Actual_Triples", "Actual_HR", "Actual_Runs", "Actual_RBI", "Actual_Walks",
        "Actual_HBP", "Actual_SB", "Actual_FS", "ResultStatus", "Notes",
    ]
    existing = [column for column in preferred if column in snapshot.columns]
    extras = [column for column in snapshot.columns if column not in existing and column != "Rank"]
    return normalize_fantasy_history(snapshot[existing + extras])


def combine_fantasy_history_uploads(uploaded_files: list[object]) -> tuple[pd.DataFrame, list[str]]:
    frames: list[pd.DataFrame] = []
    errors: list[str] = []
    for uploaded in uploaded_files or []:
        try:
            ...
            if frame.empty:
                errors.append(f"{uploaded.name}: empty file")
                continue
            frames.append(normalize_fantasy_history(frame))
        except Exception as exc:
            errors.append(f"{getattr(uploaded, 'name', 'file')}: {type(exc).__name__}: {exc}")
    if not frames:
        return pd.DataFrame(), errors
    combined = pd.concat(frames, ignore_index=True, sort=False)
    return dedupe_fantasy_history(combined), errors


def dedupe_fantasy_history(history: pd.DataFrame) -> pd.DataFrame:
    normalized = normalize_fantasy_history(history)
    if normalized.empty:
        return normalized
    keys = [column for column in ["GamePK", "player_id", "GeneratedAtUTC"] if column in normalized.columns]
    if len(keys) >= 2:
        normalized = normalized.sort_values("GeneratedAtUTC", na_position="first").drop_duplicates(keys, keep="last")
    return normalized.reset_index(drop=True)


def append_fantasy_snapshot(history: pd.DataFrame, snapshot: pd.DataFrame) -> pd.DataFrame:
    frames = [frame for frame in [history, snapshot] if frame is not None and not frame.empty]
    if not frames:
        return pd.DataFrame()
    return dedupe_fantasy_history(pd.concat(frames, ignore_index=True, sort=False))


def latest_fantasy_snapshots(
    history: pd.DataFrame,
    use_latest: bool = True,
    exclude_after_start: bool = True,
) -> pd.DataFrame:
    result = normalize_fantasy_history(history)
    if result.empty:
        return result
    if exclude_after_start:
        result = result[~result["SnapshotAfterStart"].fillna(False)].copy()
    if use_latest and not result.empty:
        result = result.sort_values("GeneratedAtUTC", na_position="first")
        has_game = result["GamePK"].notna()
        with_game = result[has_game].drop_duplicates(["GamePK", "player_id"], keep="last")
        without_game = result[~has_game].drop_duplicates(["SlateDate", "player_id"], keep="last")
        result = pd.concat([with_game, without_game], ignore_index=True, sort=False)
    return result.reset_index(drop=True)


@st.cache_data(ttl=900, max_entries=96, show_spinner=False)
def fetch_fantasy_game_boxscore(game_pk: int) -> dict:
    url = f"https://statsapi.mlb.com/api/v1.1/game/{int(game_pk)}/feed/live"
    request = Request(url, headers={"User-Agent": "MLB-Hitter-Fantasy-Backtest/1.0", "Accept": "application/json"})
    try:
        with urlopen(request, timeout=15) as response:
            payload = json.load(response)
    except (HTTPError, URLError, TimeoutError, ValueError, OSError) as exc:
        return {"ok": False, "final": False, "error": str(exc), "players": {}}

    status = payload.get("gameData", {}).get("status", {}) or {}
    ...
    final = abstract.lower() == "final" or coded.upper() == "F" or detailed.lower() in {
        "final", "game over", "completed early"
    }
    players: dict[int, dict] = {}
    teams = payload.get("liveData", {}).get("boxscore", {}).get("teams", {}) or {}
    for side in ("away", "home"):
        team_players = (teams.get(side, {}) or {}).get("players", {}) or {}
        for player in team_players.values():
            ...
            if not isinstance(batting, dict) or not batting:
                continue
            values = {
                "Actual_PA": _fs_numeric(batting.get("plateAppearances"), 0.0),
                "Actual_AB": _fs_numeric(batting.get("atBats"), 0.0),
                "Actual_Hits": _fs_numeric(batting.get("hits"), 0.0),
                "Actual_Doubles": _fs_numeric(batting.get("doubles"), 0.0),
                "Actual_Triples": _fs_numeric(batting.get("triples"), 0.0),
                "Actual_HR": _fs_numeric(batting.get("homeRuns"), 0.0),
                "Actual_Runs": _fs_numeric(batting.get("runs"), 0.0),
                "Actual_RBI": _fs_numeric(batting.get("rbi"), 0.0),
                "Actual_Walks": _fs_numeric(batting.get("baseOnBalls"), 0.0),
                "Actual_HBP": _fs_numeric(batting.get("hitByPitch"), 0.0),
                "Actual_SB": _fs_numeric(batting.get("stolenBases"), 0.0),
            }
            values["Actual_Singles"] = max(
                values["Actual_Hits"] - values["Actual_Doubles"] - values["Actual_Triples"] - values["Actual_HR"],
                0.0,
            )
            values["Player_Result_Name"] = str(person.get("fullName") or "")
            values["Appeared"] = bool(
                values["Actual_PA"] > 0
                or values["Actual_Runs"] > 0
                or values["Actual_SB"] > 0
                or values["Actual_HBP"] > 0
            )
            players[int(player_id)] = values
    return {
        "ok": True, "final": final, "status": detailed or abstract,
        "players": players, "error": "",
    }


def fill_fantasy_actual_results(history: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    result = normalize_fantasy_history(history)
    summary = {"games_checked": 0, "matched": 0, "not_final": 0, "did_not_play": 0, "unmatched": 0, "errors": 0}
    if result.empty:
        return result, summary
    pending = result[result["Actual_FS"].isna() & result["GamePK"].notna()].copy()
    game_ids = sorted({int(value) for value in pd.to_numeric(pending["GamePK"], errors="coerce").dropna().tolist()})
    if not game_ids:
        return result, summary

    progress = st.progress(0.0, text="Checking official MLB results...")
    total = len(game_ids)
    for game_index, game_pk in enumerate(game_ids, start=1):
        summary["games_checked"] += 1
        boxscore = fetch_fantasy_game_boxscore(game_pk)
        game_mask = pd.to_numeric(result["GamePK"], errors="coerce").eq(game_pk) & result["Actual_FS"].isna()
        if not boxscore.get("ok"):
            summary["errors"] += 1
            progress.progress(game_index / total, text=f"Checked {game_index} of {total} games")
            continue
        if not boxscore.get("final"):
            summary["not_final"] += 1
            progress.progress(game_index / total, text=f"Checked {game_index} of {total} games")
            continue
        player_results = boxscore.get("players", {})
        for row_index in result.index[game_mask]:
            player_id = pd.to_numeric(pd.Series([result.at[row_index, "player_id"]]), errors="coerce").iloc[0]
            if pd.isna(player_id) or int(player_id) not in player_results:
                result.at[row_index, "ResultStatus"] = "Final — player not found"
                summary["unmatched"] += 1
                continue
            actual = player_results[int(player_id)]
            if not actual.get("Appeared"):
                result.at[row_index, "ResultStatus"] = "Final — did not bat"
                summary["did_not_play"] += 1
                continue
            for column, value in actual.items():
                if column in result.columns and column not in {"Appeared", "Player_Result_Name"}:
                    result.at[row_index, column] = value
            result.at[row_index, "Actual_FS"] = _fantasy_score_from_row(result.loc[row_index])
            result.at[row_index, "ResultStatus"] = "Final"
            summary["matched"] += 1
        progress.progress(game_index / total, text=f"Checked {game_index} of {total} games")
    progress.empty()
    return normalize_fantasy_history(result), summary


def fantasy_projection_metrics(history: pd.DataFrame) -> dict:
    required = ["Projected_FS", "Actual_FS"]
    if history is None or history.empty or any(column not in history.columns for column in required):
        return {"N": 0}
    frame = history[required].apply(pd.to_numeric, errors="coerce").dropna()
    if frame.empty:
        return {"N": 0}
    projected = frame["Projected_FS"].to_numpy(float)
    actual = frame["Actual_FS"].to_numpy(float)
    error = projected - actual
    absolute = np.abs(error)
    actual_mean = float(np.mean(actual))
    baseline_mae = float(np.mean(np.abs(actual_mean - actual)))
    correlation = float(np.corrcoef(projected, actual)[0, 1]) if len(frame) >= 3 and np.std(projected) > 0 and np.std(actual) > 0 else np.nan
    return {
        "N": int(len(frame)), "MAE": float(np.mean(absolute)),
        "RMSE": float(np.sqrt(np.mean(np.square(error)))), "Bias": float(np.mean(error)),
        "Median_AE": float(np.median(absolute)), "Correlation": correlation,
        "Within_2": float(np.mean(absolute <= 2.0)), "Within_3": float(np.mean(absolute <= 3.0)),
        "Within_5": float(np.mean(absolute <= 5.0)), "Baseline_MAE": baseline_mae,
    }


def fantasy_rating_bucket_table(history: pd.DataFrame) -> pd.DataFrame:
    required = ["FantasyScoreRating", "Projected_FS", "Actual_FS"]
    if history is None or history.empty or any(column not in history.columns for column in required):
        return pd.DataFrame()
    frame = history.copy()
    for column in required + ["PP_Line"]:
        if column in frame.columns:
            frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=required)
    if frame.empty:
        return pd.DataFrame()
    frame["Rating_Bucket"] = pd.cut(
        frame["FantasyScoreRating"], bins=[-0.001, 50, 70, 85, 100.001],
        labels=["0–49", "50–69", "70–84", "85–100"], right=False, include_lowest=True,
    )
    frame["Absolute_Error"] = (frame["Projected_FS"] - frame["Actual_FS"]).abs()
    frame["Over_Result"] = np.where(
        frame["PP_Line"].notna() & ~np.isclose(frame["Actual_FS"], frame["PP_Line"]),
        frame["Actual_FS"] > frame["PP_Line"], np.nan,
    )
    return frame.groupby("Rating_Bucket", observed=False).agg(
        Sample=("Actual_FS", "size"), Average_Rating=("FantasyScoreRating", "mean"),
        Average_Projection=("Projected_FS", "mean"), Average_Actual=("Actual_FS", "mean"),
        MAE=("Absolute_Error", "mean"), Over_Rate=("Over_Result", "mean"),
    ).reset_index()


def fantasy_projection_bucket_table(history: pd.DataFrame) -> pd.DataFrame:
    if history is None or history.empty:
        return pd.DataFrame()
    frame = history.copy()
    for column in ["Projected_FS", "Actual_FS"]:
        frame[column] = pd.to_numeric(frame.get(column), errors="coerce")
    frame = frame.dropna(subset=["Projected_FS", "Actual_FS"])
    if frame.empty:
        return pd.DataFrame()
    frame["Projection_Bucket"] = pd.cut(
        frame["Projected_FS"], bins=[-0.001, 8, 10, 12, 14, 17, 100],
        labels=["<8", "8–9.9", "10–11.9", "12–13.9", "14–16.9", "17+"],
        right=False, include_lowest=True,
    )
    frame["Absolute_Error"] = (frame["Projected_FS"] - frame["Actual_FS"]).abs()
    return frame.groupby("Projection_Bucket", observed=False).agg(
        Sample=("Actual_FS", "size"), Average_Projection=("Projected_FS", "mean"),
        Average_Actual=("Actual_FS", "mean"), MAE=("Absolute_Error", "mean"),
    ).reset_index()


def fantasy_prop_records(history: pd.DataFrame) -> pd.DataFrame:
    required = ["PP_Line", "PP_Over_Prob", "PP_Under_Prob", "Actual_FS"]
    if history is None or history.empty or any(column not in history.columns for column in required):
        return pd.DataFrame()
    frame = history.copy()
    for column in required + ["Projected_FS", "FantasyScoreRating", "ProjectionEdge"]:
        frame[column] = pd.to_numeric(frame.get(column), errors="coerce")
    frame = frame.dropna(subset=required)
    if frame.empty:
        return pd.DataFrame()
    frame["Push"] = np.isclose(frame["Actual_FS"], frame["PP_Line"])
    frame["Actual_Over"] = frame["Actual_FS"] > frame["PP_Line"]
    frame["NoPushProbability"] = frame["PP_Over_Prob"] + frame["PP_Under_Prob"]
    frame = frame[frame["NoPushProbability"] > 0].copy()
    frame["PreferredSide"] = np.where(frame["PP_Over_Prob"] >= frame["PP_Under_Prob"], "Over", "Under")
    frame["PreferredProbability"] = frame[["PP_Over_Prob", "PP_Under_Prob"]].max(axis=1) / frame["NoPushProbability"]
    frame["DecisionOutcome"] = np.where(
        frame["Push"], np.nan,
        np.where(
            frame["PreferredSide"].eq("Over"), frame["Actual_FS"] > frame["PP_Line"],
            frame["Actual_FS"] < frame["PP_Line"],
        ).astype(float),
    )
    frame["ProjectionEdge"] = frame["ProjectionEdge"].fillna(frame["Projected_FS"] - frame["PP_Line"])
    return frame


def fantasy_probability_calibration_table(records: pd.DataFrame) -> tuple[pd.DataFrame, float]:
    if records is None or records.empty:
        return pd.DataFrame(), np.nan
    frame = records.dropna(subset=["PreferredProbability", "DecisionOutcome"]).copy()
    if frame.empty:
        return pd.DataFrame(), np.nan
    frame["Probability_Bucket"] = pd.cut(
        frame["PreferredProbability"], bins=[0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 1.001],
        labels=["50–54%", "55–59%", "60–64%", "65–69%", "70–74%", "75%+"],
        right=False, include_lowest=True,
    )
    grouped = frame.groupby("Probability_Bucket", observed=False).agg(
        Sample=("DecisionOutcome", "size"), Average_Probability=("PreferredProbability", "mean"),
        Actual_Win_Rate=("DecisionOutcome", "mean"),
    ).reset_index()
    grouped["Calibration_Gap"] = grouped["Actual_Win_Rate"] - grouped["Average_Probability"]
    brier = float(np.mean(np.square(frame["PreferredProbability"] - frame["DecisionOutcome"])))
    return grouped, brier


def fantasy_probability_rating_matrix(records: pd.DataFrame) -> pd.DataFrame:
    if records is None or records.empty:
        return pd.DataFrame()
    frame = records.dropna(subset=["PreferredProbability", "DecisionOutcome", "FantasyScoreRating"]).copy()
    if frame.empty:
        return pd.DataFrame()
    frame["Rating_Bucket"] = pd.cut(
        frame["FantasyScoreRating"], bins=[-0.001, 70, 85, 100.001],
        labels=["0–69", "70–84", "85–100"], right=False, include_lowest=True,
    )
    frame["Probability_Bucket"] = pd.cut(
        frame["PreferredProbability"], bins=[0.50, 0.55, 0.60, 0.65, 1.001],
        labels=["50–54%", "55–59%", "60–64%", "65%+"], right=False, include_lowest=True,
    )
    grouped = frame.groupby(["Rating_Bucket", "Probability_Bucket"], observed=False).agg(
        Sample=("DecisionOutcome", "size"), Win_Rate=("DecisionOutcome", "mean"),
    ).reset_index()
    grouped["Cell"] = grouped.apply(
        lambda row: "—" if int(row["Sample"]) == 0 else f"{row['Win_Rate']:.1%} (n={int(row['Sample'])})",
        axis=1,
    )
    return grouped.pivot(index="Rating_Bucket", columns="Probability_Bucket", values="Cell").reset_index()


def fantasy_edge_bucket_table(records: pd.DataFrame) -> pd.DataFrame:
    if records is None or records.empty:
        return pd.DataFrame()
    frame = records.dropna(subset=["ProjectionEdge", "Actual_FS", "PP_Line"]).copy()
    if frame.empty:
        return pd.DataFrame()
    frame["Edge_Bucket"] = pd.cut(
        frame["ProjectionEdge"], bins=[-100, -5, -3, -1, 0, 1, 3, 5, 100],
        labels=["≤-5", "-5 to -3", "-3 to -1", "-1 to 0", "0 to +1", "+1 to +3", "+3 to +5", "+5+"],
        include_lowest=True,
    )
    return frame.groupby("Edge_Bucket", observed=False).agg(
        Sample=("Actual_FS", "size"), Average_Projection=("Projected_FS", "mean"),
        Average_Line=("PP_Line", "mean"), Average_Actual=("Actual_FS", "mean"),
        Over_Rate=("Actual_Over", "mean"), Decision_Win_Rate=("DecisionOutcome", "mean"),
    ).reset_index()


def fantasy_segment_metrics(history: pd.DataFrame, group_column: str) -> pd.DataFrame:
    if history is None or history.empty or group_column not in history.columns:
        return pd.DataFrame()
    rows: list[dict] = []
    for group_value, group in history.groupby(group_column, dropna=False):
        metrics = fantasy_projection_metrics(group)
        if not metrics.get("N"):
            continue
        decisions = fantasy_prop_records(group).dropna(subset=["DecisionOutcome"])
        rows.append({
            "Group": str(group_value), "N": metrics["N"], "MAE": metrics["MAE"],
            "RMSE": metrics["RMSE"], "Bias": metrics["Bias"], "Correlation": metrics["Correlation"],
            "Decision_N": int(len(decisions)),
            "Decision_Win_Rate": float(decisions["DecisionOutcome"].mean()) if not decisions.empty else np.nan,
        })
    return pd.DataFrame(rows)


def fit_fantasy_linear_calibration(history: pd.DataFrame, min_rows: int = 100) -> dict:
    if history is None or history.empty:
        return {"ok": False, "n": 0, "message": "No completed fantasy-score outcomes are available."}
    frame = history[["Projected_FS", "Actual_FS", "SlateDate", "GeneratedAtUTC"]].copy()
    for column in ["Projected_FS", "Actual_FS"]:
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame["SlateDate"] = pd.to_datetime(frame["SlateDate"], errors="coerce")
    frame["GeneratedAtUTC"] = pd.to_datetime(frame["GeneratedAtUTC"], utc=True, errors="coerce")
    frame = frame.dropna(subset=["Projected_FS", "Actual_FS"]).sort_values(["SlateDate", "GeneratedAtUTC"])
    if len(frame) < min_rows or frame["Projected_FS"].nunique() < 3:
        return {"ok": False, "n": int(len(frame)), "message": f"Need at least {min_rows} completed hitter-games with varied fantasy projections."}
    x = frame["Projected_FS"].to_numpy(float)
    y = frame["Actual_FS"].to_numpy(float)
    slope, intercept = np.polyfit(x, y, 1)
    slope = float(np.clip(slope, 0.10, 3.0))
    ...
    result = {
        ...
        "calibrated_rmse": float(np.sqrt(np.mean(np.square(calibrated - y)))),
        "holdout_n": 0, "holdout_raw_mae": np.nan, "holdout_calibrated_mae": np.nan,
        "holdout_raw_rmse": np.nan, "holdout_calibrated_rmse": np.nan,
    }
    split = max(int(len(frame) * 0.70), 1)
    train, test = frame.iloc[:split], frame.iloc[split:]
    if len(train) >= max(50, min_rows // 2) and len(test) >= 20 and train["Projected_FS"].nunique() >= 3:
        train_slope, train_intercept = np.polyfit(train["Projected_FS"].to_numpy(float), train["Actual_FS"].to_numpy(float), 1)
        train_slope = float(np.clip(train_slope, 0.10, 3.0))
        test_x = test["Projected_FS"].to_numpy(float)
        test_y = test["Actual_FS"].to_numpy(float)
        test_cal = float(train_intercept) + train_slope * test_x
        result.update({
            "holdout_n": int(len(test)),
            "holdout_raw_mae": float(np.mean(np.abs(test_x - test_y))),
            "holdout_calibrated_mae": float(np.mean(np.abs(test_cal - test_y))),
            "holdout_raw_rmse": float(np.sqrt(np.mean(np.square(test_x - test_y)))),
            "holdout_calibrated_rmse": float(np.sqrt(np.mean(np.square(test_cal - test_y)))),
        })
    return result


def render_fantasy_backtest(
    board: pd.DataFrame,
    ...
    st.caption(
        "Save pregame projections and PrizePicks lines, then fetch official MLB results after games become final. "
        "The scoring values used for each snapshot are stored with every row so historical fantasy scores remain reproducible."
    )

    generated_at = pd.Timestamp.now(tz="UTC").isoformat()
    current_snapshot = build_fantasy_snapshot(board, comparison, scoring, generated_at)
    snapshot_csv = current_snapshot.to_csv(index=False).encode("utf-8")
    st.download_button(
        "Download current pregame snapshot", snapshot_csv,
        file_name=f"hitter_fs_snapshot_{pd.Timestamp(current_snapshot['SlateDate'].dropna().iloc[0]).date() if current_snapshot['SlateDate'].notna().any() else date.today()}.csv",
        mime="text/csv", width="stretch",
    )

    uploaded_history = st.file_uploader(
        "Upload fantasy-score snapshot or master-history CSV files",
        type=["csv"], accept_multiple_files=True, key="fs_backtest_uploads",
    )
    if uploaded_history:
        upload_signature = tuple((uploaded.name, len(uploaded.getvalue())) for uploaded in uploaded_history)
        if st.session_state.get("fs_backtest_upload_signature") != upload_signature:
            uploaded_frame, upload_errors = combine_fantasy_history_uploads(uploaded_history)
            existing = normalize_fantasy_history(st.session_state.get("fs_backtest_history", pd.DataFrame()))
            st.session_state["fs_backtest_history"] = append_fantasy_snapshot(existing, uploaded_frame)
            st.session_state["fs_backtest_upload_signature"] = upload_signature
            st.session_state["fs_backtest_upload_errors"] = upload_errors

    history = normalize_fantasy_history(st.session_state.get("fs_backtest_history", pd.DataFrame()))
    upload_errors = st.session_state.get("fs_backtest_upload_errors", [])
    if upload_errors:
    ...
    with action_columns[0]:
        if st.button("Add current snapshot to session master", width="stretch", key="fs_add_snapshot"):
            history = append_fantasy_snapshot(history, current_snapshot)
            st.session_state["fs_backtest_history"] = history
            st.success("Current pregame snapshot added. Download the updated master CSV before leaving the app.")
    with action_columns[1]:
        if st.button("Fetch official completed-game results", type="primary", width="stretch", key="fs_fetch_results"):
            history, result_summary = fill_fantasy_actual_results(history)
            st.session_state["fs_backtest_history"] = history
            st.session_state["fs_backtest_result_summary"] = result_summary

    result_summary = st.session_state.get("fs_backtest_result_summary", {})
    if result_summary:
        st.caption(
            f"Result update: {result_summary.get('matched', 0)} hitter rows matched · "
            f"{result_summary.get('not_final', 0)} games not final · "
            f"{result_summary.get('did_not_play', 0)} did not bat · "
            f"{result_summary.get('unmatched', 0)} unmatched · {result_summary.get('errors', 0)} errors"
        )

    history = normalize_fantasy_history(st.session_state.get("fs_backtest_history", history))
    if not history.empty:
        st.download_button(
            "Download updated master history", history.to_csv(index=False).encode("utf-8"),
            file_name="hitter_fs_master_history.csv", mime="text/csv", width="stretch",
        )

    c1, c2 = st.columns(2)
    use_latest = c1.checkbox("Use latest pregame snapshot per player/game", value=True, key="fs_bt_latest")
    exclude_after = c2.checkbox("Exclude snapshots saved after first pitch", value=True, key="fs_bt_exclude_after")
    analysis = latest_fantasy_snapshots(history, use_latest=use_latest, exclude_after_start=exclude_after)
    completed = analysis[pd.to_numeric(analysis.get("Actual_FS"), errors="coerce").notna()].copy() if not analysis.empty else pd.DataFrame()

    cards = st.columns(4)
    cards[0].metric("Master rows", f"{len(history):,}")
    cards[1].metric("Analysis rows", f"{len(analysis):,}")
    cards[2].metric("Completed outcomes", f"{len(completed):,}")
    cards[3].metric("After-start rows", f"{int(history.get('SnapshotAfterStart', pd.Series(dtype=bool)).fillna(False).sum()):,}" if not history.empty else "0")

    if completed.empty:
        st.info("No completed fantasy-score outcomes are available yet. Save pregame snapshots and fetch results after games are final.")
        return

    st.markdown("### 1. Projection accuracy")
    metrics = fantasy_projection_metrics(completed)
    metric_table = pd.DataFrame([metrics])
    st.dataframe(
        metric_table.style.format({
            "MAE": "{:.3f}", "RMSE": "{:.3f}", "Bias": "{:+.3f}", "Median_AE": "{:.3f}",
            "Correlation": "{:.3f}", "Within_2": "{:.1%}", "Within_3": "{:.1%}",
            "Within_5": "{:.1%}", "Baseline_MAE": "{:.3f}",
        }), width="stretch", hide_index=True,
    )
    st.caption("Bias is projected fantasy score minus actual fantasy score. Positive means the model is projecting too high.")

    st.markdown("### 2. Projection buckets")
    projection_table = fantasy_projection_bucket_table(completed)
    st.dataframe(
        projection_table.style.format({
            "Average_Projection": "{:.2f}", "Average_Actual": "{:.2f}", "MAE": "{:.2f}",
        }), width="stretch", hide_index=True,
    )

    st.markdown("### 3. FS Rating validation")
    rating_table = fantasy_rating_bucket_table(completed)
    st.dataframe(
        rating_table.style.format({
            "Average_Rating": "{:.1f}", "Average_Projection": "{:.2f}",
            "Average_Actual": "{:.2f}", "MAE": "{:.2f}", "Over_Rate": "{:.1%}",
        }), width="stretch", hide_index=True,
    )
    st.caption("FS Rating is slate-relative. This table tests whether higher rating buckets actually produce better fantasy outcomes.")

    records = fantasy_prop_records(completed)
    st.markdown("### 4. PrizePicks probability calibration")
    if records.empty:
        st.info("No completed rows contain both a saved PrizePicks line and simulated over/under probabilities.")
    else:
        decision_rows = records.dropna(subset=["DecisionOutcome"])
        calibration_table, brier = fantasy_probability_calibration_table(records)
        p1, p2, p3 = st.columns(3)
        p1.metric("Decisions", f"{len(decision_rows):,}")
        p2.metric("Pushes", f"{int(records['Push'].sum()):,}")
        p3.metric("Brier score", f"{brier:.3f}" if pd.notna(brier) else "—", help="Lower is better; 0 is perfect.")
        st.dataframe(
            calibration_table.style.format({
                "Average_Probability": "{:.1%}", "Actual_Win_Rate": "{:.1%}",
                "Calibration_Gap": "{:+.1%}",
            }), width="stretch", hide_index=True,
        )

        st.markdown("### 5. Preferred probability × FS Rating")
        st.caption("Each cell is the actual win rate for the model's preferred Over or Under side. Cells with fewer than 10 decisions are exploratory.")
        matrix = fantasy_probability_rating_matrix(records)
        st.dataframe(matrix, width="stretch", hide_index=True)

        st.markdown("### 6. Projection edge versus line")
        edge_table = fantasy_edge_bucket_table(records)
        st.dataframe(
            edge_table.style.format({
                "Average_Projection": "{:.2f}", "Average_Line": "{:.2f}",
                "Average_Actual": "{:.2f}", "Over_Rate": "{:.1%}",
                "Decision_Win_Rate": "{:.1%}",
            }), width="stretch", hide_index=True,
        )
    ...
    segment_options = [
        column for column in ["Confidence_Level", "LineupStatus", "LineupSource", "LineupSpot", "Team", "Opponent"]
        if column in completed.columns
    ]
    if segment_options:
        selected_segment = st.selectbox("Break results down by", segment_options, key="fs_bt_segment")
        segment_table = fantasy_segment_metrics(completed, selected_segment)
        st.dataframe(
            segment_table.style.format({
                "MAE": "{:.3f}", "RMSE": "{:.3f}", "Bias": "{:+.3f}",
                "Correlation": "{:.3f}", "Decision_Win_Rate": "{:.1%}",
            }), width="stretch", hide_index=True,
        )

    st.markdown("### 8. Fit fantasy-score calibration")
    minimum_rows = st.number_input(
        "Minimum completed hitter-games", min_value=50, max_value=5000, value=100, step=25,
        key="fs_bt_min_calibration",
    )
    fit = fit_fantasy_linear_calibration(completed, int(minimum_rows))
    if not fit.get("ok"):
        st.info(fit.get("message", "Not enough completed outcomes."))
    else:
        fit_table = pd.DataFrame([{key: value for key, value in fit.items() if key not in {"ok", "message"}}])
        st.dataframe(
            fit_table.style.format({
                "slope": "{:.4f}", "intercept": "{:+.4f}", "raw_mae": "{:.4f}",
                "calibrated_mae": "{:.4f}", "raw_rmse": "{:.4f}",
                "calibrated_rmse": "{:.4f}", "holdout_raw_mae": "{:.4f}",
                "holdout_calibrated_mae": "{:.4f}", "holdout_raw_rmse": "{:.4f}",
                "holdout_calibrated_rmse": "{:.4f}",
            }), width="stretch", hide_index=True,
        )
        st.caption("The chronological holdout fits coefficients on the earlier 70% of hitter-games and tests them on the later 30%. Do not apply calibration unless holdout error improves.")
        calibration_payload = {
            "version": FS_BACKTEST_VERSION,
            "created_at_utc": pd.Timestamp.now(tz="UTC").isoformat(),
            "target": "Projected_FS",
            "method": "actual_equals_intercept_plus_slope_times_projection",
            "intercept": fit["intercept"], "slope": fit["slope"], "n": fit["n"],
        }
        st.download_button(
            "Download fantasy-score calibration JSON",
            json.dumps(calibration_payload, indent=2).encode("utf-8"),
            file_name="hitter_fs_calibration.json", mime="application/json", width="stretch",
        )

    with st.expander("Completed history used in this report"):
        show_columns = [
            "SlateDate", "Player", "Team", "Opponent", "Projected_FS", "Actual_FS",
            "FantasyScoreRating", "PP_Line", "PP_Over_Prob", "PP_Under_Prob",
            "PreferredSide", "PreferredProbability", "Confidence_Level", "LineupStatus",
            "ResultStatus",
        ]
        st.dataframe(
            completed[[column for column in show_columns if column in completed.columns]].sort_values(
                ["SlateDate", "Projected_FS"], ascending=[False, False]
...
if build_board:
    with st.spinner("Building lineup, matchup, weather and fantasy-score projections..."):
        ...
        st.session_state["fs_board_generated_at"] = pd.Timestamp.now(tz="UTC").isoformat()
...
(
    ...
    run_tab, pitch_tab, bvp_tab, environment_tab, backtest_tab, notes_tab,
) = st.tabs([
    ...
    "Lineups & environment", "Backtest & calibration", "Model notes",
])
...
with board_tab:
    ...
    st.caption("Heatmap text may switch between dark and white automatically for contrast. Text color is not a separate model signal or recommendation.")
...
with backtest_tab:
    render_fantasy_backtest(board, comparison, scoring)
...
with notes_tab:
    st.markdown(
        ...
        - White or dark text inside heatmap cells is selected automatically for readability; it is not an extra signal.
        - **Backtest & calibration** saves pregame projections and scoring settings, retrieves official MLB batting results, validates FS Rating and PrizePicks probabilities, and runs a chronological calibration holdout.
        - Download the updated master-history CSV after every result update because Streamlit session storage can reset.
