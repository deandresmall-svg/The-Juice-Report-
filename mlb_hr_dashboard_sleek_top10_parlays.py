...
def apply_slate_hr_probability_calibration(snapshot: pd.DataFrame, calibration: dict | None) -> pd.DataFrame:
    """Apply the saved HR probability calibration to a whole-slate snapshot table."""
    result = snapshot.copy() if snapshot is not None else pd.DataFrame()
    if result.empty:
        return result
    raw = pd.to_numeric(
        result.get("RawProbability", result.get("ModelProbability")),
        errors="coerce",
    ).clip(0.001, 0.999)
    if "RawProbability" not in result.columns:
        result["RawProbability"] = raw
    result["Calibration_Applied"] = False
    result["Calibration_Version"] = ""
    if calibration:
        try:
            ...
            slope = float(calibration.get("slope"))
            calibrated = _sigmoid(intercept + slope * _logit(raw))
            result["ModelProbability"] = np.clip(calibrated, 0.001, 0.999)
            ...
            result["ModelProbability"] = pd.to_numeric(result.get("ModelProbability"), errors="coerce")
    result["ModelProbability"] = pd.to_numeric(result.get("ModelProbability"), errors="coerce")
    result["ModelScore"] = pd.to_numeric(result.get("ModelScore", result.get("HRScore")), errors="coerce")
    result["Confidence"] = pd.to_numeric(result.get("Confidence"), errors="coerce")
    return result


def hr_target_tier(probability: object, score: object) -> str:
    """Readable HR tier using the user's working HR targeting cheat sheet."""
    ...
    if p >= 0.14 and 55 <= s < 70:
        return "Lean only"
    if 0.11 <= p < 0.14 and s >= 85:
        return "Longshot only"
    if p < 0.11 or s < 45:
        return "Avoid"
    return "Monitor"


def add_hr_slate_grade(
    board: pd.DataFrame,
    probability_weight: float,
    score_weight: float,
    confidence_weight: float,
) -> pd.DataFrame:
    """Rank a whole slate using HR probability, HR score, and confidence."""
    result = board.copy() if board is not None else pd.DataFrame()
    if result.empty:
        return result
    probability = pd.to_numeric(result.get("ModelProbability"), errors="coerce")
    score = pd.to_numeric(result.get("ModelScore", result.get("HRScore")), errors="coerce").fillna(50.0).clip(0, 100)
    ...
    ) / max(float(probability_weight + score_weight + confidence_weight), 1.0)
    result["TargetTier"] = [
        hr_target_tier(probability_value, score_value)
        for probability_value, score_value in zip(probability, score)
    ]
    result = result.sort_values(
        ["SlateGrade", "ModelProbability", "ModelScore"],
        ascending=False,
...
def generate_three_man_hr_pairings(
    board: pd.DataFrame,
    ...
    """Generate three-player HR combinations from the whole-slate board."""
    if board is None or board.empty:
        return pd.DataFrame()
    candidates = board.copy()
    candidates["ModelProbability"] = pd.to_numeric(candidates.get("ModelProbability"), errors="coerce")
    candidates["ModelScore"] = pd.to_numeric(candidates.get("ModelScore", candidates.get("HRScore")), errors="coerce")
    candidates["Confidence"] = pd.to_numeric(candidates.get("Confidence"), errors="coerce").fillna(50.0)
    candidates = candidates[
        candidates["ModelProbability"].ge(float(minimum_probability))
        & candidates["ModelScore"].ge(float(minimum_score))
        & candidates["Confidence"].ge(float(minimum_confidence))
    ].copy()
    if "PlayerID" in candidates.columns:
        candidates = candidates.drop_duplicates(["PlayerID", "GamePK"], keep="first")
    else:
        candidates = candidates.drop_duplicates(["Player", "GamePK"], keep="first")
    candidates = candidates.sort_values(
        ["SlateGrade", "ModelProbability", "ModelScore"],
        ascending=False,
    ).head(int(candidate_pool)).reset_index(drop=True)
    if len(candidates) < 3:
        return pd.DataFrame()

    rows: list[dict] = []
    for indices in combinations(range(len(candidates)), 3):
        trio = candidates.iloc[list(indices)].copy()
        ...
        if require_different_games and unique_games < 3:
            continue
        probabilities = pd.to_numeric(trio["ModelProbability"], errors="coerce").clip(0.001, 0.999)
        scores = pd.to_numeric(trio.get("ModelScore", trio.get("HRScore")), errors="coerce").fillna(50.0)
        slate_grades = pd.to_numeric(trio.get("SlateGrade"), errors="coerce").fillna(50.0)
        confidence = pd.to_numeric(trio.get("Confidence"), errors="coerce").fillna(50.0)
        if probabilities.isna().any():
            continue
        joint_probability = float(np.prod(probabilities.to_numpy()))
        geometric_probability = float(joint_probability ** (1.0 / 3.0))
        avg_probability = float(probabilities.mean())
        avg_score = float(scores.mean())
        avg_grade = float(slate_grades.mean())
        min_confidence = float(confidence.min())
        tier_bonus = float(trio.get("TargetTier", pd.Series(["Monitor"] * 3)).astype(str).isin(["Best target", "Strong target", "Playable target"]).mean() * 8.0)
        if style == "Highest probability":
            raw_score = 55.0 * geometric_probability + 0.25 * avg_score + 0.20 * avg_grade + tier_bonus
        elif style == "Model score":
            raw_score = 35.0 * geometric_probability + 0.45 * avg_score + 0.20 * avg_grade + tier_bonus
        elif style == "Longshot upside":
            raw_score = 25.0 * geometric_probability + 0.50 * avg_score + 0.15 * avg_grade + tier_bonus
        else:
            raw_score = 45.0 * geometric_probability + 0.30 * avg_score + 0.25 * avg_grade + tier_bonus
        if unique_games < 3:
            raw_score -= (3 - unique_games) * 2.5
        if not team_counts.empty and int(team_counts.max()) > 1:
            raw_score -= (int(team_counts.max()) - 1) * 3.0

        key_column = "PlayerID" if "PlayerID" in trio.columns else "Player"
        player_keys = tuple(f"{trio.iloc[pos].get(key_column, trio.iloc[pos].get('Player', ''))}|{trio.iloc[pos].get('GamePK', '')}" for pos in range(3))

        def leg_text(position: int) -> str:
            row = trio.iloc[position]
            p = float(probabilities.iloc[position])
            s = float(scores.iloc[position])
            team = str(row.get("Team", ""))
            opp = str(row.get("Opponent", ""))
            pitcher = str(row.get("StartingPitcher", ""))
            return f"{p:.1%} · Score {s:.0f} · {team} vs {opp} ({pitcher})"
        ...
        rows.append(
            {
                ...
                "AverageLegProbability": avg_probability,
                "AverageHRScore": avg_score,
                ...
                "Games": " | ".join(
                    (
                        trio.get("AwayTeam", pd.Series([""] * 3)).astype(str)
                        + " @ "
                        + trio.get("HomeTeam", pd.Series([""] * 3)).astype(str)
                    ).tolist()
                ),
            }
        )
...
def render_hr_top10_parlay_tab(
    ...
    st.markdown("### Whole-slate Top 10 & 3-man HR parlay generator")
    st.caption(
        "Builds both offenses for every game on the slate, ranks the best HR targets, and creates optional three-leg combinations. "
        "The all-three probability is a simple independence estimate, so use it for comparison—not certainty."
    )
    default_date = str(pd.Timestamp(matchup.get("slate_date") or date.today()).date())
    controls = st.columns([1.1, 1.0, 1.0, 1.2])
    with controls[0]:
        slate_date_input = st.date_input(
            "Slate date",
            value=pd.to_datetime(default_date).date(),
            key=f"{widget_prefix}_top10_slate_date",
        )
    with controls[1]:
        use_confirmed_lineups = st.checkbox(
            "Use confirmed lineups",
            value=True,
            key=f"{widget_prefix}_top10_confirmed_lineups",
        )
    with controls[2]:
        min_prob_filter = st.slider(
            ...
            0.01,
            format="%.0%%",
            key=f"{widget_prefix}_top10_min_prob_filter",
        )
    with controls[3]:
        min_score_filter = st.slider(
            ...
            45,
            5,
            key=f"{widget_prefix}_top10_min_score_filter",
        )

    slate_date_text = str(pd.Timestamp(slate_date_input).date())
    board_key = f"{widget_prefix}_top10_slate_board_{slate_date_text}"
    messages_key = f"{widget_prefix}_top10_slate_messages_{slate_date_text}"
    if st.button(
        "Build / refresh whole-slate Top 10 + parlay pool",
        type="primary",
        use_container_width=True,
        key=f"{widget_prefix}_top10_build_button",
    ):
        with st.spinner("Building whole-slate HR board..."):
            slate_snapshot, messages = build_entire_slate_hr_snapshot(
                df=df,
                pitcher_summary=pitcher_summary,
                available_teams=available_teams,
                slate_date_text=slate_date_text,
        ...
        st.session_state[board_key] = slate_snapshot
    ...
    score_weight = weight_columns[1].slider("HR score weight", 0, 100, 35, 5, key=f"{widget_prefix}_top10_score_weight")
    confidence_weight = weight_columns[2].slider("Confidence weight", 0, 100, 15, 5, key=f"{widget_prefix}_top10_conf_weight")
    ranked_slate = add_hr_slate_grade(slate_board, probability_weight, score_weight, confidence_weight)
    ranked_slate = ranked_slate[
        pd.to_numeric(ranked_slate.get("ModelProbability"), errors="coerce").ge(float(min_prob_filter))
        & pd.to_numeric(ranked_slate.get("ModelScore"), errors="coerce").ge(float(min_score_filter))
    ].copy()
    if ranked_slate.empty:
        st.warning("No hitters met the current probability/score filters. Lower the cutoffs or rebuild the slate.")
        return
    ...
    gradient_columns = [column for column in ["HR Score", "Confidence", "Slate Grade"] if column in top10.columns]
    if gradient_columns:
        display_style = display_style.background_gradient(cmap="RdYlGn", subset=gradient_columns, vmin=0, vmax=100)
    st.dataframe(
        display_style.format(
            {
                "Order": "{:.0f}",
                "Proj PA": "{:.2f}",
                "Exp HR": "{:.3f}",
    ...
    st.download_button(
        "Download whole-slate HR board",
        ranked_slate.to_csv(index=False).encode("utf-8"),
        file_name=f"hr_whole_slate_top10_pool_{slate_date_text}.csv",
        mime="text/csv",
        use_container_width=True,
        key=f"{widget_prefix}_top10_download_board",
    )

    st.markdown("#### 3-man HR parlay generator")
    c1, c2, c3, c4 = st.columns(4)
    pair_count = c1.slider("Number of pairings", 1, 20, 10, 1, key=f"{widget_prefix}_pair_count")
    max_pool = max(6, min(40, len(ranked_slate)))
    pool_size = c2.slider("Candidate pool", 6, max_pool, min(18, max_pool), 1, key=f"{widget_prefix}_pool_size")
    pair_min_prob = c3.slider("Pairing min HR prob", 0.00, 0.30, float(min_prob_filter), 0.01, format="%.0%%", key=f"{widget_prefix}_pair_min_prob")
    pair_min_score = c4.slider("Pairing min HR score", 0, 100, max(55, int(min_score_filter)), 5, key=f"{widget_prefix}_pair_min_score")
    c5, c6, c7, c8 = st.columns(4)
    pair_min_confidence = c5.slider("Minimum confidence", 0, 100, 45, 5, key=f"{widget_prefix}_pair_min_conf")
    max_same_team = c6.selectbox("Max hitters from one team", [1, 2, 3], index=0, key=f"{widget_prefix}_pair_max_team")
    require_different_games = c7.checkbox("Require three different games", value=True, key=f"{widget_prefix}_pair_diff_games")
    style = c8.selectbox("Pairing style", ["Balanced", "Highest probability", "Model score", "Longshot upside"], key=f"{widget_prefix}_pair_style")
    diversity = st.slider("Player-exposure diversity", 0.0, 10.0, 3.0, 0.5, key=f"{widget_prefix}_pair_diversity")

    pairings = generate_three_man_hr_pairings(
        ranked_slate,
        number_of_pairings=pair_count,
        candidate_pool=pool_size,
        minimum_probability=pair_min_prob,
        minimum_score=pair_min_score,
        minimum_confidence=pair_min_confidence,
        max_same_team=max_same_team,
        require_different_games=require_different_games,
        style=style,
        diversity_strength=diversity,
    )
    if pairings.empty:
        st.warning("No valid 3-man combinations met the current restrictions. Increase the pool or loosen the filters.")
        return
    pairing_columns = [
        "Pairing", "Player 1", "Leg 1", "Player 2", "Leg 2", "Player 3", "Leg 3",
        "PairingScore", "EstimatedAll3Probability", "AverageLegProbability", "AverageHRScore",
        "MinimumConfidence", "Games",
    ]
    pairing_view = pairings[[column for column in pairing_columns if column in pairings.columns]].copy()
    pairing_view = pairing_view.rename(
        columns={
            "PairingScore": "Pairing Score",
            "EstimatedAll3Probability": "Est. All 3 HR",
            "AverageLegProbability": "Avg Leg Prob",
            "AverageHRScore": "Avg HR Score",
            "MinimumConfidence": "Min Confidence",
        }
    )
    pair_style = pairing_view.style
    pair_gradient = [column for column in ["Pairing Score", "Avg HR Score", "Min Confidence"] if column in pairing_view.columns]
    if pair_gradient:
        pair_style = pair_style.background_gradient(cmap="RdYlGn", subset=pair_gradient, axis=0)
    st.dataframe(
        pair_style.format(
            {
                "Pairing Score": "{:.1f}",
    ...
    st.caption(
        ...
        "Same-game/team outcomes can be correlated, so use the list as a ranking tool."
    )
    ...
    st.markdown(
        ...
        <div class="edge-topbar">
            <div class="edge-brand">
                ...
                <div>
                    <div class="edge-brand-title">HR EDGE</div>
                    <div class="edge-brand-subtitle">Home run targeting & backtest lab</div>
                </div>
            </div>
            <div class="edge-nav">
                <span class="edge-nav-item active">Dashboard</span><span class="edge-nav-item">Matchups</span><span class="edge-nav-item">Hitters</span><span class="edge-nav-item">Lineups</span><span class="edge-nav-item">Park Factors</span><span class="edge-nav-item">Backtest</span><span class="edge-nav-item">Settings</span>
            </div>
            <div class="edge-meta">
                <span>☾</span>
