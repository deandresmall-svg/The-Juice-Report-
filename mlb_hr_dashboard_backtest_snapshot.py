...
def fit_hr_logistic_calibration(history: pd.DataFrame) -> dict | None:
    valid, _ = _binary_backtest_metrics(history)
    if len(valid) < 50 or valid["Actual_Event"].nunique(dropna=True) < 2:
        return None
    p = pd.to_numeric(valid["ModelProbability"], errors="coerce").clip(0.001, 0.999)
...
def render_hr_backtest_calibration_tab(
    ...
    st.caption("Save pregame HR probabilities, build an entire-slate snapshot, then upload scored history to check calibration.")
    ...
    with c1:
        ...
        if st.button("Add current matchup to master snapshot", key=f"{widget_prefix}_add_current_snapshot", width="stretch"):
    ...
    if not master.empty:
        ...
        if st.button("Clear in-session master snapshot", key=f"{widget_prefix}_clear_master_snapshot"):
            st.session_state[f"{widget_prefix}_master_snapshot"] = pd.DataFrame()
            st.rerun()

    st.markdown("#### Upload history / score results")
    ...
    for uploaded in uploads or []:
        try:
            history_frames.append(pd.read_csv(uploaded))
        except Exception as exc:
    ...
    if not history.empty:
        scored = score_hr_snapshot_with_statcast(history, df)
        scored = normalize_hr_history(scored)
        st.download_button(
            "Download scored history CSV",
            scored.to_csv(index=False).encode("utf-8"),
            file_name=f"hr_scored_history_{slate_text}.csv",
            mime="text/csv",
            width="stretch",
        )
        valid, metrics = _binary_backtest_metrics(scored)
        if metrics:
            ...
            bucketed = valid.copy()
            bucketed["Probability Bucket"] = pd.cut(
                pd.to_numeric(bucketed["ModelProbability"], errors="coerce"),
                bins=[0, .05, .08, .11, .14, .17, .20, .25, 1.0],
                labels=["0-5%", "5-8%", "8-11%", "11-14%", "14-17%", "17-20%", "20-25%", "25%+"],
                include_lowest=True,
            )
            by_bucket = bucketed.groupby("Probability Bucket", observed=True).agg(
                Rows=("Actual_Event", "size"),
                Avg_Model=("ModelProbability", "mean"),
                Actual_Rate=("Actual_Event", "mean"),
                Avg_Score=("ModelScore", "mean"),
            ).reset_index()
            st.dataframe(
                by_bucket.style.format({"Avg_Model": "{:.1%}", "Actual_Rate": "{:.1%}", "Avg_Score": "{:.1f}"}),
                use_container_width=True,
                hide_index=True,
            )

            calibration = fit_hr_logistic_calibration(scored)
            if calibration:
            ...
            st.info("No completed pregame rows were available for metrics yet. Upload a scored history CSV or include games contained in the loaded Statcast sample.")

    st.markdown("#### Apply a saved calibration")
