...
def fetch_savant_park_factors(venue: str, year: int, metric: str) -> dict:
    """Best-effort live Savant lookup with a safe neutral fallback."""
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = {
            side: executor.submit(_fetch_savant_side_factor, venue, int(year), side, metric)
            for side in ("L", "R")
        }
        results = {side: futures[side].result() for side in ("L", "R")}

    left, left_source = results["L"]
    right, right_source = results["R"]
    errors = [message for _, message in results.values() if message and "Baseball Savant" not in message]
    sources = [message for _, message in results.values() if message and "Baseball Savant" in message]
    return {
        ...
        "source": sources[0] if sources else "Neutral fallback",
...
with st.expander("Game context and model adjustments", expanded=True):
    ...
    with c2:
        ...
        if manual_park_override:
            ...
            if not auto_park["ok"]:
                st.warning("Automatic Savant factors were unavailable, so neutral 100 values are being used. Turn on the manual override to change them.")
