...
def prepare_data(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    barrel_min_la = np.where(ev.ge(116), 8.0, 26.0 - barrel_expansion)
    barrel_max_la = np.where(ev.ge(116), 50.0, 30.0 + barrel_expansion)
    df["Dynamic_Barrel"] = (
        ev.ge(98) & la.ge(barrel_min_la) & la.le(barrel_max_la)
    ).fillna(False).astype(int)

    df["LA_Deviation_from_Optimal"] = (df["launch_angle"] - 27.0).abs()
    df["LA_Optimization_Score"] = (100.0 - df["LA_Deviation_from_Optimal"] * 3.0).clip(0, 100)
