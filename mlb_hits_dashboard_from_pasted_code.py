...
rankings, pitch_mix = build_hit_board(
    ...
    lineup_override=pd.DataFrame(), lineup_edits=None, sprint_upload=None,
...
with contact_tab:
    ...
    contact_style = contact_style.set_properties(subset=["Player"], **{"font-weight": "700", "background-color": "#f8fafc"}).format({
...
with matchup_tab:
    ...
    st.dataframe(detail.style.background_gradient(cmap="RdYlGn", subset=score_cols, vmin=0, vmax=100).format(
        {column: "{:.1f}" for column in score_cols}), use_container_width=True, hide_index=True, height=520)
...
with bvp_tab:
    ...
    bvp_style = bvp_style.set_properties(subset=["Player"], **{"font-weight": "700", "background-color": "#f8fafc"}).format({
