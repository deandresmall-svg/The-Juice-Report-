...
MODEL_VERSION = "pitcher-lab-backtest-v1"
BACKTEST_TARGETS = {
    "Strikeouts": {
...
def normalize_backtest_history(history: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_columns = {
        ...
        "Outs_Closing_Line", "Actual_K", "Actual_ER", "Actual_Outs", "Actual_BF", "Actual_Pitches",
    }
    for column in numeric_columns:
        if column not in result.columns:
...
def fill_actual_results(history: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    result = normalize_backtest_history(history)
    summary = {"matched": 0, "unmatched": 0, "errors": []}
    if result.empty or "PitcherID" not in result.columns:
        return result, summary
    candidates = result[result["PitcherID"].notna() & pd.Series(result["SlateDate"]).notna()].copy()
    pairs = candidates[["PitcherID", "SlateDate"]].drop_duplicates()
    ...
    for index, pair in enumerate(pairs.itertuples(index=False), start=1):
        ...
        try:
            season = int(pd.Timestamp(slate_day).year)
            logs, error = fetch_pitcher_game_log(pitcher_id, season)
            if error and (logs is None or logs.empty):
                summary["errors"].append(f"{pitcher_id} {slate_day}: {error}")
                summary["unmatched"] += 1
                progress.progress(index / total, text=f"Checked {index} of {total} pitcher-days")
                continue
            game_rows = logs[pd.to_datetime(logs["Date"], errors="coerce").dt.date.eq(slate_day)].copy()
            if game_rows.empty:
                summary["unmatched"] += 1
            else:
                actual = game_rows.sort_values(["Pitches", "Outs"], ascending=False).iloc[0]
                mask = result["PitcherID"].eq(pitcher_id) & pd.Series(result["SlateDate"]).eq(slate_day)
                mapping = {
                    "Actual_K": actual.get("K"), "Actual_ER": actual.get("ER"), "Actual_Outs": actual.get("Outs"),
                    "Actual_BF": actual.get("BF"), "Actual_Pitches": actual.get("Pitches"),
                }
                for column, value in mapping.items():
                    result.loc[mask & result[column].isna(), column] = pd.to_numeric(pd.Series([value]), errors="coerce").iloc[0]
                summary["matched"] += int(mask.sum())
            progress.progress(index / total, text=f"Checked {index} of {total} pitcher-days")
        except Exception as exc:
            summary["errors"].append(f"{pitcher_id} {slate_day}: {type(exc).__name__}: {exc}")
            summary["unmatched"] += 1
            progress.progress(index / total, text=f"Checked {index} of {total} pitcher-days")
    progress.empty()
    return result, summary
...
with backtest_tab:
    ...
    if history.empty:
        st.info("Download and save pregame snapshots first. Upload them here after games finish to build the calibration sample.")
    else:
        ...
        if st.button("Fetch official completed-game results", type="primary", width="stretch"):
            updated_history, result_summary = fill_actual_results(history)
            st.session_state["pitcher_backtest_history"] = updated_history
        ...
        if result_summary:
            st.caption(
                ...
                f"{result_summary.get('unmatched', 0)} pitcher-days not yet matched."
            )
        ...
        counts = st.columns(4)
        counts[0].metric("Uploaded rows", f"{len(history):,}")
...
with notes_tab:
    st.markdown(
        ...
        - Download the updated master-history CSV after every result update. Streamlit Community Cloud storage is temporary, so the downloaded file is your persistent record.
