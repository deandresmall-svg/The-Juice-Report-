...
def build_hr_board(
    ...
    board["Projected_HR"] = board["Model_HR_Per_PA"] * board["Projected_PA"]
...
def binary_score_probability_joint_table(
    ...
    """Cross-check HR score buckets inside active probability buckets.

    When an active calibration is available, reapply it to RawProbability for
    every completed historical row. This keeps old and new snapshots on one
    comparable probability scale instead of mixing raw and calibrated values.
    """
    ...
    # Four score tiers and HR probability bands keep the two-way cells useful
    # while matching the probability buckets already shown above this section.
    score_bins = [-0.001, 45, 55, 70, 100.001]
    score_labels = ["0–44", "45–54", "55–69", "70–100"]
    probability_bins = [0, .08, .11, .14, .17, .20, .25, 1.001]
    probability_labels = [
        "<8%", "8–10%", "11–13%", "14–16%", "17–19%", "20–24%", "25%+"
    ]
    ...
    grouped["Calibration_Gap"] = (
        grouped["Actual_HR_Rate"] - grouped["Average_Probability"]
    )
    grouped["Score_Lift_Within_Probability"] = (
        grouped["Actual_HR_Rate"] - grouped["Probability_Bucket_Rate"]
    )
    ...
    display["Cell"] = display.apply(
        lambda row: (
            "—"
            if int(row["Sample"]) == 0 or pd.isna(row["Actual_HR_Rate"])
            else f"{float(row['Actual_HR_Rate']):.1%} (n={int(row['Sample'])})"
        ),
        axis=1,
    )
    ...
    matrix.index.name = "HR score bucket"
    matrix.columns.name = "Active HR probability bucket"
...
def render_binary_backtest_tab(
    ...
    editable = editable.rename(columns={
        ...
        "ExpectedCount": "Projected HR", "ModelProbability": "1+ HR",
    ...
    st.markdown("#### 5. HR probability × HR score")
    ...
    if joint_matrix.empty:
        st.info("Not enough completed rows are available for the combined HR probability-and-score check.")
    else:
        st.caption(
            "Each cell is the actual home-run rate with its sample size. "
        ...
        with st.expander("Detailed HR probability × score results"):
            detail_columns = [
                "Score_Bucket", "Probability_Bucket", "Sample", "Homers",
                "Average_Probability", "Actual_HR_Rate", "Calibration_Gap",
            ...
            st.caption(
                "Score lift within probability compares each score cell's HR rate with the overall HR rate "
                "of that same probability bucket. Positive lift means HR Score added separation beyond probability."
            )
...
with quick_tab:
    quick_columns = [
        "Rank", "Player", "LineupSpot", "Projected_PA", "Projected_HR", "Model_1plus_HR",
    ...
    quick = quick.rename(columns={
        "LineupSpot": "Order", "Projected_PA": "Proj PA", "Projected_HR": "Proj HR", "Model_1plus_HR": "1+ HR",
    ...
    styler = quick.style.background_gradient(
        ...
        "Order": "{:.0f}", "Proj PA": "{:.2f}", "Proj HR": "{:.2f}", "1+ HR": "{:.1%}", "Raw 1+ HR": "{:.1%}",
...
with backtest_tab:
    ...
    render_binary_backtest_tab(
        ...
        whole_slate_board=st.session_state.get(
            f"clean_hr_slate_whole_slate_board_{str(pd.Timestamp(matchup.get('slate_date') or date.today()).date())}",
            pd.DataFrame(),
        ),
        odds_api_key=odds_api_key,
        odds_widget_prefix="clean_hr",
...
with notes_tab:
    st.markdown(
        ...
        - **Projected HR** is the expected event count: modeled rate per PA × projected PA. It is not itself a probability.
