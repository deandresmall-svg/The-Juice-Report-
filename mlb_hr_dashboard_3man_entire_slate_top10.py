...
def _hr_probability_bucket_table(valid: pd.DataFrame) -> pd.DataFrame:
    """Summarize actual HR results by model-probability bucket."""
    data = _completed_hr_probability_rows(valid)
    if data.empty:
        return pd.DataFrame(
            columns=["Probability_Bucket", "Sample", "HRs", "Average_Probability", "Actual_HR_Rate", "Calibration_Gap"]
        )
    data["Probability_Bucket"] = pd.cut(
        data["_p"],
        bins=[0.0, 0.05, 0.08, 0.11, 0.14, 0.17, 0.20, 0.25, 1.0],
        labels=["0-5%", "5-8%", "8-11%", "11-14%", "14-17%", "17-20%", "20-25%", "25%+"],
        include_lowest=True,
    )
    table = (
        data.dropna(subset=["Probability_Bucket"])
        .groupby("Probability_Bucket", observed=True)
        .agg(
            Sample=("_y", "size"),
            HRs=("_y", "sum"),
            Average_Probability=("_p", "mean"),
            Actual_HR_Rate=("_y", "mean"),
        )
        .reset_index()
    )
    if table.empty:
        return table
    table["HRs"] = pd.to_numeric(table["HRs"], errors="coerce").fillna(0).astype(int)
    table["Calibration_Gap"] = table["Actual_HR_Rate"] - table["Average_Probability"]
    return table


def _hr_score_bucket_table(valid: pd.DataFrame) -> pd.DataFrame:
    """Summarize actual HR results by HRScore bucket."""
    data = _completed_hr_probability_rows(valid)
    if data.empty or "ModelScore" not in data.columns:
        return pd.DataFrame(
            columns=["Score_Bucket", "Sample", "HRs", "Average_Probability", "Actual_HR_Rate", "Average_HR_Score", "Calibration_Gap"]
        )
    data["_score"] = pd.to_numeric(data.get("ModelScore"), errors="coerce")
    data = data[data["_score"].notna()].copy()
    if data.empty:
        return pd.DataFrame()
    data["Score_Bucket"] = pd.cut(
        data["_score"],
        bins=[-0.001, 30, 45, 55, 70, 85, np.inf],
        labels=["0-29", "30-44", "45-54", "55-69", "70-84", "85-100"],
        include_lowest=True,
    )
    table = (
        data.dropna(subset=["Score_Bucket"])
...
def _render_hr_bucket_tables(valid: pd.DataFrame) -> None:
    """Render probability and HR-score buckets in the same style as the pitcher dashboard."""
    data = _completed_hr_probability_rows(valid)
    if data.empty:
        return

    st.markdown("#### Probability calibration buckets")
    decisions = int(len(data))
    brier = float(np.mean((data["_p"].astype(float) - data["_y"].astype(float)) ** 2)) if decisions else np.nan
    c1, c2 = st.columns(2)
    c1.metric("Decisions", f"{decisions:,}")
    c2.metric("Brier score", f"{brier:.3f}" if pd.notna(brier) else "—")

    prob_bucket = _hr_probability_bucket_table(valid)
    prob_formatters = {
    ...
    if prob_bucket.empty:
        ...
        st.dataframe(prob_bucket.style.format(prob_formatters), use_container_width=True, hide_index=True)

    st.markdown("#### Score buckets")
    st.selectbox("Score target", ["Home runs"], index=0, key="hr_score_bucket_target")
    score_bucket = _hr_score_bucket_table(valid)
    score_formatters = {
        "Average_Probability": "{:.1%}",
        "Actual_HR_Rate": "{:.1%}",
        "Average_HR_Score": "{:.1f}",
    ...
    if score_bucket.empty:
        ...
        st.dataframe(score_bucket.style.format(score_formatters), use_container_width=True, hide_index=True)
...
def _render_hr_accuracy_and_calibration_tables(valid: pd.DataFrame, min_rows: int = 30) -> None:
    ...
    if not accuracy.empty:
        ...
        st.caption("Bias is model probability minus actual HR result. Positive bias means the model is projecting too high.")
...
def _format_player_leg(row: pd.Series) -> str:
    """Compact player label used in parlay recommendations."""
    player = str(row.get("Player", "")).strip() or f"MLB ID {row.get('player_id', '')}"
    team = str(row.get("Team", "")).strip()
    prob = pd.to_numeric(pd.Series([row.get("Model_1plus_HR")]), errors="coerce").iloc[0]
    score = pd.to_numeric(pd.Series([row.get("HRScore")]), errors="coerce").iloc[0]
    parts = [player]
    if team:
        parts.append(f"({team})")
    details = []
    if pd.notna(prob):
        details.append(f"{prob:.1%}")
    if pd.notna(score):
        details.append(f"Score {score:.0f}")
    return " ".join(parts) + (" · " + " / ".join(details) if details else "")


def _hr_parlay_read(min_prob: float, min_score: float, avg_prob: float, avg_score: float) -> str:
    if min_prob >= 0.17 and min_score >= 70:
        return "Elite profile"
    if min_prob >= 0.14 and min_score >= 70:
        return "Strong profile"
    if min_prob >= 0.11 and min_score >= 85:
        return "High-score longshot"
    if avg_prob >= 0.14 and avg_score >= 70:
        return "Playable mix"
    return "Longshot / review odds"


def build_three_man_hr_parlays(
    board: pd.DataFrame,
    min_probability: float = 0.14,
    min_score: float = 70.0,
    longshot_probability: float = 0.11,
    longshot_score: float = 85.0,
    max_pool: int = 14,
    max_recommendations: int = 14,
    diversify_teams: bool = False,
    require_medium_confidence: bool = False,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Generate ranked 3-leg HR parlay recommendations from the current board.

    The main pool uses the current cheat-sheet rule: HR probability >= 14% and
    HRScore >= 70.  A secondary longshot path allows 11%+ probabilities when
    HRScore is 85+, because that score bucket has been the strongest in the
    dashboard's current backtest.
    """
    ...
    # Allow the parlay builder to work from either the live board or an
    # entire-slate snapshot/master-history frame.  Snapshot rows store the
    # same values under ModelProbability/RawProbability/ModelScore/PlayerID.
    if "Model_1plus_HR" not in pool.columns and "ModelProbability" in pool.columns:
        pool["Model_1plus_HR"] = pool["ModelProbability"]
    if "Raw_Model_1plus_HR" not in pool.columns and "RawProbability" in pool.columns:
        pool["Raw_Model_1plus_HR"] = pool["RawProbability"]
    if "HRScore" not in pool.columns and "ModelScore" in pool.columns:
        pool["HRScore"] = pool["ModelScore"]
    if "player_id" not in pool.columns and "PlayerID" in pool.columns:
        pool["player_id"] = pool["PlayerID"]

    for column in [
        "Model_1plus_HR", "Raw_Model_1plus_HR", "ModelProbability", "RawProbability", "ModelScore", "HRScore", "LA_Power_Score",
        "PitchMatchScore", "ZoneFitScore", "PitcherSideAttackScore", "Adj_Brl_PA",
        "Projected_PA", "LineupSpot",
    ]:
        if column in pool.columns:
            pool[column] = pd.to_numeric(pool[column], errors="coerce")

    if "Model_1plus_HR" not in pool.columns or "HRScore" not in pool.columns:
        return pd.DataFrame(), pd.DataFrame()

    pool = pool.dropna(subset=["Model_1plus_HR", "HRScore"]).copy()
    pool = pool[pool["Model_1plus_HR"].gt(0) & pool["HRScore"].gt(0)].copy()
    if require_medium_confidence and "Confidence_Level" in pool.columns:
        pool = pool[pool["Confidence_Level"].isin(["Medium", "High"])].copy()

    primary = pool[
        pool["Model_1plus_HR"].ge(float(min_probability))
        & pool["HRScore"].ge(float(min_score))
    ].copy()
    longshot = pool[
        pool["Model_1plus_HR"].ge(float(longshot_probability))
        & pool["HRScore"].ge(float(longshot_score))
    ].copy()
    candidates = pd.concat([primary, longshot], ignore_index=True)
    candidates = candidates.drop_duplicates("player_id", keep="first") if "player_id" in candidates.columns else candidates.drop_duplicates("Player", keep="first")

    # If the strict pool is too small, relax just enough to show useful choices.
    if len(candidates) < 3:
        fallback = pool[
            pool["Model_1plus_HR"].ge(max(0.08, float(min_probability) - 0.03))
            & pool["HRScore"].ge(max(55.0, float(min_score) - 15.0))
        ].copy()
    ...
    candidates["CandidateScore"] = (
        candidates["Model_1plus_HR"] * 100.0 * 1.50
        + candidates["HRScore"] * 0.55
        + pd.to_numeric(candidates.get("PitchMatchScore", 50), errors="coerce").fillna(50.0) * 0.15
        + pd.to_numeric(candidates.get("ZoneFitScore", 50), errors="coerce").fillna(50.0) * 0.10
        + pd.to_numeric(candidates.get("PitcherSideAttackScore", 50), errors="coerce").fillna(50.0) * 0.10
    )
    candidates = candidates.sort_values(
        ["CandidateScore", "Model_1plus_HR", "HRScore"], ascending=False
    ).head(int(max_pool)).reset_index(drop=True)

    rows: list[dict] = []
    for combo in combinations(list(candidates.index), 3):
        legs = candidates.loc[list(combo)].copy()
        if diversify_teams and "Team" in legs.columns and legs["Team"].nunique(dropna=True) < 3:
            continue
        probs = pd.to_numeric(legs["Model_1plus_HR"], errors="coerce").fillna(0.0).clip(0, 1)
        scores = pd.to_numeric(legs["HRScore"], errors="coerce").fillna(0.0)
        combo_probability = float(np.prod(probs.to_numpy(float)))
        avg_probability = float(probs.mean())
        min_probability_leg = float(probs.min())
        avg_score = float(scores.mean())
        min_score_leg = float(scores.min())
        avg_pitch_match = float(pd.to_numeric(legs.get("PitchMatchScore", 50), errors="coerce").fillna(50.0).mean())
        avg_zone_fit = float(pd.to_numeric(legs.get("ZoneFitScore", 50), errors="coerce").fillna(50.0).mean())
        avg_side_attack = float(pd.to_numeric(legs.get("PitcherSideAttackScore", 50), errors="coerce").fillna(50.0).mean())
        avg_la = float(pd.to_numeric(legs.get("LA_Power_Score", 50), errors="coerce").fillna(50.0).mean())
        avg_barrel = float(pd.to_numeric(legs.get("Adj_Brl_PA", np.nan), errors="coerce").mean())
        quality = (
            avg_score * 0.42
            + min_score_leg * 0.22
            + avg_probability * 100.0 * 1.30
            + min_probability_leg * 100.0 * 0.90
            + avg_pitch_match * 0.08
            + avg_zone_fit * 0.05
            + avg_side_attack * 0.05
            + avg_la * 0.05
        )
        rows.append(
            {
                "Leg 1": _format_player_leg(legs.iloc[0]),
                "Leg 2": _format_player_leg(legs.iloc[1]),
                "Leg 3": _format_player_leg(legs.iloc[2]),
                "Combined Model %": combo_probability,
                "Avg HR %": avg_probability,
                "Min Leg HR %": min_probability_leg,
                "Avg HR Score": avg_score,
                "Min HR Score": min_score_leg,
                "Avg Pitch Match": avg_pitch_match,
                "Avg Zone Fit": avg_zone_fit,
                "Avg Side Attack": avg_side_attack,
                "Avg LA Power": avg_la,
                "Avg Adj Brl/PA": avg_barrel,
                "Parlay Score": quality,
                "Read": _hr_parlay_read(min_probability_leg, min_score_leg, avg_probability, avg_score),
            }
        )

    parlays = pd.DataFrame(rows)
    if parlays.empty:
        return candidates, parlays
    parlays = parlays.sort_values(
        ["Parlay Score", "Combined Model %", "Avg HR Score"], ascending=False
    ).head(int(max_recommendations)).reset_index(drop=True)
    parlays.insert(0, "Parlay #", np.arange(1, len(parlays) + 1))
    return candidates, parlays



def build_top_entire_slate_hr_targets(board: pd.DataFrame, top_n: int = 10) -> pd.DataFrame:
    """Return the top full-slate HR targets using probability + HR score.

    This works with either the live board columns or the entire-slate snapshot
    columns created by build_hr_projection_snapshot.
    """
    if board is None or board.empty:
        return pd.DataFrame()

    top = board.copy()
    if "Model_1plus_HR" not in top.columns and "ModelProbability" in top.columns:
        top["Model_1plus_HR"] = top["ModelProbability"]
    if "Raw_Model_1plus_HR" not in top.columns and "RawProbability" in top.columns:
        top["Raw_Model_1plus_HR"] = top["RawProbability"]
    if "HRScore" not in top.columns and "ModelScore" in top.columns:
        top["HRScore"] = top["ModelScore"]
    if "player_id" not in top.columns and "PlayerID" in top.columns:
        top["player_id"] = top["PlayerID"]

    numeric_columns = [
        "Model_1plus_HR", "Raw_Model_1plus_HR", "HRScore", "LA_Power_Score",
        "Adj_Brl_PA", "PitchMatchScore", "ZoneFitScore", "PitcherSideAttackScore",
        "PitcherPowerScore", "ParkFactor", "Projected_PA", "LineupSpot",
    ]
    for column in numeric_columns:
        if column in top.columns:
            top[column] = pd.to_numeric(top[column], errors="coerce")

    if "Model_1plus_HR" not in top.columns or "HRScore" not in top.columns:
        return pd.DataFrame()

    top = top.dropna(subset=["Model_1plus_HR", "HRScore"]).copy()
    top = top[top["Model_1plus_HR"].gt(0) & top["HRScore"].gt(0)].copy()
    if top.empty:
        return pd.DataFrame()

    # One player can appear more than once if the slate has a doubleheader or
    # if a saved history was appended twice. Keep the strongest current row.
    top["Top10Score"] = (
        top["Model_1plus_HR"].clip(0, 1) * 100.0 * 1.60
        + top["HRScore"] * 0.55
        + pd.to_numeric(top.get("PitchMatchScore", 50), errors="coerce").fillna(50.0) * 0.12
        + pd.to_numeric(top.get("ZoneFitScore", 50), errors="coerce").fillna(50.0) * 0.08
        + pd.to_numeric(top.get("PitcherSideAttackScore", 50), errors="coerce").fillna(50.0) * 0.08
        + pd.to_numeric(top.get("LA_Power_Score", 50), errors="coerce").fillna(50.0) * 0.07
    )
    dedupe_key = "player_id" if "player_id" in top.columns else "Player"
    if dedupe_key in top.columns:
        top = top.sort_values(["Top10Score", "Model_1plus_HR", "HRScore"], ascending=False)
        top = top.drop_duplicates(dedupe_key, keep="first")

    top = top.sort_values(["Top10Score", "Model_1plus_HR", "HRScore"], ascending=False).head(int(top_n)).reset_index(drop=True)
    top.insert(0, "Slate Rank", np.arange(1, len(top) + 1))
    return top


def render_top_entire_slate_hr_targets(board: pd.DataFrame, slate_text: str, widget_prefix: str = "hr") -> None:
    top10 = build_top_entire_slate_hr_targets(board, top_n=10)
    if top10.empty:
        st.info("No top-10 slate targets are available yet. Build the entire-slate pool first.")
        return

    st.markdown("#### Top 10 entire-slate HR targets")
    st.caption("Ranked by a blended probability + HR Score formula, with pitch match, zone fit, side attack, and launch-angle power used as tie-breakers.")

    display_cols = [
        "Slate Rank", "Player", "Team", "Opponent", "StartingPitcher", "LineupSpot",
        "Model_1plus_HR", "HRScore", "Top10Score", "LA_Power_Score", "Adj_Brl_PA",
        "PitchMatchScore", "ZoneFitScore", "PitcherSideAttackScore", "PitcherPowerScore",
        "ParkFactor", "Confidence_Level", "SampleStatus",
    ]
    top_view = top10[[column for column in display_cols if column in top10.columns]].copy()
    top_view = top_view.rename(columns={
        "LineupSpot": "Order",
        "Model_1plus_HR": "HR %",
        "HRScore": "HR Score",
        "Top10Score": "Top 10 Score",
        "LA_Power_Score": "LA Power",
        "Adj_Brl_PA": "Adj Brl/PA",
        "PitchMatchScore": "Pitch Match",
        "ZoneFitScore": "Zone Fit",
        "PitcherSideAttackScore": "Side Attack",
        "PitcherPowerScore": "Pitcher Power",
    ...
    score_cols = [
        c for c in ["HR Score", "Top 10 Score", "LA Power", "Pitch Match", "Zone Fit", "Side Attack", "Pitcher Power", "Park Factor"]
        if c in top_view.columns
    ]
    style = top_view.style
    if score_cols:
        style = style.background_gradient(cmap="RdYlGn", subset=score_cols, vmin=0, vmax=100)
    style = style.format({
        "HR %": "{:.1%}",
        "HR Score": "{:.1f}",
        "Top 10 Score": "{:.1f}",
    ...
    st.dataframe(style, use_container_width=True, hide_index=True, height=385)
    st.download_button(
        "Download top 10 entire-slate HR targets",
        top10.to_csv(index=False).encode("utf-8"),
        file_name=f"top_10_entire_slate_hr_targets_{slate_text}.csv",
        mime="text/csv",
        key=f"{widget_prefix}_top10_slate_download",
        width="stretch",
    )

def render_three_man_hr_parlay_tab(
    board: pd.DataFrame,
    df: pd.DataFrame | None = None,
    matchup: dict | None = None,
    pitcher_summary: pd.DataFrame | None = None,
    available_teams: list[str] | None = None,
    lookback_days: int | None = None,
    loaded_start: object | None = None,
    loaded_end: object | None = None,
    min_pa: int = 75,
    include_low_sample: bool = False,
    bat_tracking_auto: pd.DataFrame | None = None,
    widget_prefix: str = "hr",
) -> None:
    st.subheader("Automatic 3-man HR parlays")
    st.caption(
        "Build 3-leg HR parlay ideas from either the selected matchup or the entire MLB slate. "
        "The rankings use HR probability, HR Score, pitch match, zone fit, pitcher-side attack, "
        "launch-angle power, and barrel rate. Use odds/price as the final filter before playing anything."
    )

    matchup = matchup or {}
    slate_text = str(pd.Timestamp(matchup.get("slate_date") or date.today()).date())
    source_options = ["Current matchup", "Entire slate"]
    source_default = 1 if not st.session_state.get(f"{widget_prefix}_entire_slate_snapshot", pd.DataFrame()).empty else 0
    source = st.radio(
        "Parlay source",
        source_options,
        index=source_default,
        horizontal=True,
        key=f"{widget_prefix}_parlay_source",
    )

    parlay_board = board.copy() if board is not None else pd.DataFrame()
    if source == "Entire slate":
        can_build_slate = all([
            df is not None and not df.empty,
            pitcher_summary is not None and not pitcher_summary.empty,
            available_teams,
            loaded_start is not None,
            loaded_end is not None,
        ])
        slate_c1, slate_c2, slate_c3 = st.columns([1.1, 1.1, .9])
        use_confirmed_lineups = slate_c1.checkbox(
            "Use confirmed lineups when available",
            value=True,
            key=f"{widget_prefix}_parlay_slate_confirmed",
        )
        build_clicked = slate_c2.button(
            "Build/update entire slate parlay pool",
            key=f"{widget_prefix}_parlay_build_entire_slate",
            width="stretch",
            disabled=not can_build_slate,
        )
        existing_slate = st.session_state.get(f"{widget_prefix}_entire_slate_snapshot", pd.DataFrame())
        if build_clicked and can_build_slate:
            with st.spinner("Building HR candidates for every game and both offenses..."):
                whole_slate, messages = build_entire_slate_hr_snapshot(
                    df=df,
                    pitcher_summary=pitcher_summary,
                    ...
                    loaded_end=loaded_end,
                    min_pa=int(min_pa),
                    include_low_sample=bool(include_low_sample),
                    bat_tracking_auto=bat_tracking_auto,
                    use_confirmed_lineups=bool(use_confirmed_lineups),
                )
            st.session_state[f"{widget_prefix}_entire_slate_snapshot"] = whole_slate
            st.session_state[f"{widget_prefix}_entire_slate_messages"] = messages
            existing_slate = whole_slate
            if whole_slate.empty:
                st.error("No entire-slate HR candidates were created. Check probables, Statcast sample, or filters.")
            else:
                st.success(f"Built {len(whole_slate):,} entire-slate hitter rows for parlay generation.")
        elif not can_build_slate:
            st.warning("Entire-slate generation is unavailable until the slate/date and Statcast inputs are loaded.")

        parlay_board = existing_slate.copy() if existing_slate is not None else pd.DataFrame()
        if parlay_board.empty:
            st.info("No entire-slate pool is built yet. Click **Build/update entire slate parlay pool** or use the Backtest tab's **Add entire slate to snapshot** button first.")
            return

        # Use the currently loaded probability calibration on the slate pool when available.
        if "Model_1plus_HR" not in parlay_board.columns and "ModelProbability" in parlay_board.columns:
            parlay_board["Model_1plus_HR"] = parlay_board["ModelProbability"]
        if "Raw_Model_1plus_HR" not in parlay_board.columns and "RawProbability" in parlay_board.columns:
            parlay_board["Raw_Model_1plus_HR"] = parlay_board["RawProbability"]
        if "HRScore" not in parlay_board.columns and "ModelScore" in parlay_board.columns:
            parlay_board["HRScore"] = parlay_board["ModelScore"]
        calibration = st.session_state.get(f"{widget_prefix}_probability_calibration")
        if calibration and {"Model_1plus_HR", "HRScore"}.issubset(parlay_board.columns):
            parlay_board = apply_hr_probability_calibration(parlay_board, calibration)
            parlay_board["ModelProbability"] = parlay_board["Model_1plus_HR"]

        slate_c3.metric("Slate hitter rows", f"{len(parlay_board):,}")
        st.download_button(
            "Download entire-slate parlay pool",
            parlay_board.to_csv(index=False).encode("utf-8"),
            file_name=f"hr_entire_slate_parlay_pool_{slate_text}.csv",
            mime="text/csv",
            key=f"{widget_prefix}_parlay_pool_download",
            width="stretch",
        )

        render_top_entire_slate_hr_targets(parlay_board, slate_text=slate_text, widget_prefix=widget_prefix)

    c1, c2, c3, c4 = st.columns(4)
    min_probability = c1.slider(
        "Minimum HR probability", 0.08, 0.25, 0.14, 0.01,
        format="%.2f", key=f"{widget_prefix}_parlay_min_prob",
    )
    min_score = c2.slider(
        "Minimum HR Score", 40, 95, 70, 1,
        key=f"{widget_prefix}_parlay_min_score",
    )
    longshot_probability = c3.slider(
        "Longshot min probability", 0.06, 0.18, 0.11, 0.01,
        format="%.2f", key=f"{widget_prefix}_parlay_longshot_prob",
    )
    longshot_score = c4.slider(
        "Longshot min HR Score", 60, 100, 85, 1,
        key=f"{widget_prefix}_parlay_longshot_score",
    )

    c5, c6, c7, c8 = st.columns(4)
    max_pool = c5.slider("Max hitters in pool", 6, 24, 14, 1, key=f"{widget_prefix}_parlay_max_pool")
    max_recommendations = c6.slider("Number of parlays", 5, 30, 14, 1, key=f"{widget_prefix}_parlay_count")
    diversify_teams = c7.checkbox("Avoid same-team combos", value=False, key=f"{widget_prefix}_parlay_diversify")
    require_medium_confidence = c8.checkbox("Require Medium/High confidence", value=False, key=f"{widget_prefix}_parlay_confidence")

    candidates, parlays = build_three_man_hr_parlays(
        parlay_board,
        min_probability=float(min_probability),
        min_score=float(min_score),
        longshot_probability=float(longshot_probability),
        longshot_score=float(longshot_score),
        max_pool=int(max_pool),
        max_recommendations=int(max_recommendations),
        diversify_teams=bool(diversify_teams),
        require_medium_confidence=bool(require_medium_confidence),
    )

    m1, m2, m3 = st.columns(3)
    m1.metric("Candidate pool", int(len(candidates)))
    m2.metric("Generated parlays", int(len(parlays)))
    if not parlays.empty:
        m3.metric("Top combo model chance", f"{float(parlays.iloc[0]['Combined Model %']):.2%}")
    else:
        m3.metric("Top combo model chance", "—")

    with st.expander("Candidate pool", expanded=False):
        candidate_cols = [
            "Player", "Team", "Model_1plus_HR", "HRScore", "LA_Power_Score", "Adj_Brl_PA",
            "PitchMatchScore", "ZoneFitScore", "PitcherSideAttackScore", "Confidence_Level", "SampleStatus",
        ]
        candidate_view = candidates[[column for column in candidate_cols if column in candidates.columns]].copy()
        if not candidate_view.empty:
            candidate_view = candidate_view.rename(columns={
                "Model_1plus_HR": "HR %", "HRScore": "HR Score", "LA_Power_Score": "LA Power",
                "Adj_Brl_PA": "Adj Brl/PA", "PitchMatchScore": "Pitch Match",
                "ZoneFitScore": "Zone Fit", "PitcherSideAttackScore": "Side Attack",
                "Confidence_Level": "Confidence", "SampleStatus": "Sample",
            })
            score_cols = [c for c in ["HR Score", "LA Power", "Pitch Match", "Zone Fit", "Side Attack"] if c in candidate_view.columns]
            style = candidate_view.style
            if score_cols:
                style = style.background_gradient(cmap="RdYlGn", subset=score_cols, vmin=0, vmax=100)
            style = style.format({"HR %": "{:.1%}", "HR Score": "{:.1f}", "LA Power": "{:.1f}", "Adj Brl/PA": "{:.2%}", "Pitch Match": "{:.1f}", "Zone Fit": "{:.1f}", "Side Attack": "{:.1f}"})
            st.dataframe(style, use_container_width=True, hide_index=True, height=380)
        else:
            st.info("No candidates matched the current filters.")

    if parlays.empty:
        st.warning("No 3-man combinations matched the current filters. Lower the probability/score filters or allow same-team combos.")
        return

    parlay_style = parlays.style.background_gradient(
        cmap="RdYlGn", subset=["Parlay Score", "Avg HR Score", "Min HR Score", "Avg Pitch Match", "Avg Zone Fit", "Avg Side Attack", "Avg LA Power"], vmin=0, vmax=100
    ).format({
        "Combined Model %": "{:.2%}",
        "Avg HR %": "{:.1%}",
        "Min Leg HR %": "{:.1%}",
    ...
    st.dataframe(parlay_style, use_container_width=True, hide_index=True, height=620)
    st.download_button(
        "Download 3-man HR parlays",
        data=parlays.to_csv(index=False).encode("utf-8"),
        file_name="three_man_hr_parlays.csv",
        mime="text/csv",
        key=f"{widget_prefix}_parlay_download",
    )
...
quick_tab, power_tab, matchup_tab, pitch_type_tab, tracking_tab, pitcher_tab, parlay_tab, backtest_tab, notes_tab = st.tabs(
    [
        "Quick board", "Power profile", "Matchup detail", "Batter vs pitch type",
        "Bat tracking", "Pitcher profile", "3-man HR parlays", "Backtest & calibration", "Model notes"
    ]
)
...
with parlay_tab:
    render_three_man_hr_parlay_tab(
        rankings,
        df=df,
        matchup=matchup,
        pitcher_summary=pitcher_summary,
