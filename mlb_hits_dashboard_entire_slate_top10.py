...
def render_slate_tools(
    ...
    st.markdown("### Top 10 entire-slate hits targets and three-person pairing suggestions")
    st.caption(
        "Builds both offenses for every game on the selected slate. The Top 10 ranks hitters by a blended "
        "slate grade using 1+ hit probability, Hit Score, and confidence. Pairings are suggestions—not guarantees—"
        "and the displayed all-three probability is an independence approximation."
    )
    slate_date = str(pd.Timestamp(matchup.get("slate_date") or date.today()).date())
    games, schedule_error = fetch_mlb_schedule(slate_date)
    if schedule_error:
    ...
    st.markdown("#### Top 10 entire-slate settings")
    ...
    score_weight = weight_columns[1].slider("Hit Score weight", 0, 100, 35, 5, key=f"{widget_prefix}_score_weight")
    confidence_weight = weight_columns[2].slider("Confidence weight", 0, 100, 20, 5, key=f"{widget_prefix}_confidence_weight")
    min_top_probability = weight_columns[3].slider("Min 1+ hit %", 0, 90, 0, 1, key=f"{widget_prefix}_top10_min_prob") / 100.0
    min_top_score = weight_columns[4].slider("Min Hit Score", 0, 100, 0, 5, key=f"{widget_prefix}_top10_min_score")
    ranked_slate = add_slate_grade(slate_board, probability_weight, score_weight, confidence_weight)
    top_pool = ranked_slate.copy()
    top_pool = top_pool[pd.to_numeric(top_pool.get(BINARY_PROBABILITY_COLUMN), errors="coerce").ge(float(min_top_probability))]
    top_pool = top_pool[pd.to_numeric(top_pool.get(BINARY_SCORE_COLUMN), errors="coerce").ge(float(min_top_score))]
    top_pool = top_pool.sort_values(
        ["SlateGrade", BINARY_PROBABILITY_COLUMN, BINARY_SCORE_COLUMN],
        ascending=False,
    ).reset_index(drop=True)
    top_pool = top_pool.drop(columns=["Top10Rank"], errors="ignore")
    if not top_pool.empty:
        top_pool.insert(0, "Top10Rank", np.arange(1, len(top_pool) + 1))
    top_columns = [
        "Top10Rank", "Player", "Team", "Opponent", "Game", "LineupSpot",
        BINARY_EXPECTED_COUNT_COLUMN, BINARY_PROBABILITY_COLUMN, "Model_2plus_Hit",
        BINARY_SCORE_COLUMN, "SlateGrade", "Confidence", "Confidence_Level",
        "Adj_xHit_PA", "Contact_Pct", "K_Pct", "PitchMatchScore", "ZoneFitScore",
        "PitcherSideAttackScore", "PitcherHitScore", "RecentFormScore", "BvPScore",
        "LineupStatus", "Market_Line", "Market_Over_Prob", "Model_Market_Edge", "Line_Source",
    ]
    top10 = top_pool[[column for column in top_columns if column in top_pool.columns]].head(10).copy()
    ...
    top_metrics[0].metric("Top 10 pool", f"{len(top_pool):,}")
    top_metrics[1].metric("Slate hitters built", f"{len(ranked_slate):,}")
    top_metrics[2].metric(
        "Highest 1+ hit",
        f"{pd.to_numeric(ranked_slate.get(BINARY_PROBABILITY_COLUMN), errors='coerce').max():.1%}"
        if pd.to_numeric(ranked_slate.get(BINARY_PROBABILITY_COLUMN), errors='coerce').notna().any() else "—",
    )
    top_metrics[3].metric(
        "Highest Hit Score",
        f"{pd.to_numeric(ranked_slate.get(BINARY_SCORE_COLUMN), errors='coerce').max():.1f}"
        if pd.to_numeric(ranked_slate.get(BINARY_SCORE_COLUMN), errors='coerce').notna().any() else "—",
    )
    format_map = {
        "Top 10 Rank": "{:.0f}", BINARY_EXPECTED_COUNT_LABEL: "{:.2f}",
        BINARY_TARGET_LABEL: "{:.1%}", "2+ Hit": "{:.1%}", "Hit Score": "{:.1f}",
        "Confidence": "{:.1f}", "SlateGrade": "{:.1f}", "Adj xHit/PA": "{:.1%}",
        "Contact%": "{:.1%}", "K%": "{:.1%}", "Pitch Match": "{:.1f}",
        "Zone Fit": "{:.1f}", "Side Attack": "{:.1f}", "Pitcher Vuln": "{:.1f}",
        "Recent Form": "{:.1f}", "BvP Score": "{:.1f}", "No-vig Market": "{:.1%}",
        "Model Edge": "{:+.1%}", "Line": "{:.1f}", "Order": "{:.0f}",
    }
    score_subsets = [
        column for column in [
            "Hit Score", "Confidence", "SlateGrade", "Pitch Match", "Zone Fit",
            "Side Attack", "Pitcher Vuln", "Recent Form", "BvP Score"
    ...
    if "K%" in top10.columns:
        top_style = top_style.background_gradient(cmap="RdYlGn_r", subset=["K%"], axis=0)
    ...
    top_download_columns = [column for column in top_columns if column in top_pool.columns]
    st.download_button(
        "Download Top 10 entire-slate hits",
        top_pool[top_download_columns].head(10).to_csv(index=False).encode("utf-8"),
        file_name=f"{BINARY_TARGET_SLUG}_top10_entire_slate_{slate_date}.csv",
        mime="text/csv",
        width="stretch",
    )
    st.download_button(
